...
def _parse_command_item(item: dict) -> CommandSpec:
    ...
    return CommandSpec(
        ...
        expect_stdout_regex=[
            str(text) for text in _normalize_list(item.get("expect_stdout_regex"))
        ],
        expect_stdout_any_of=[
            str(text) for text in _normalize_list(item.get("expect_stdout_any_of"))
        ],
        expect_stderr_contains=[
            str(text) for text in _normalize_list(item.get("expect_stderr_contains"))
        ],
        expect_error_code=(
            str(item.get("expect_error_code"))
            if item.get("expect_error_code") is not None
            else None
        ),
        expect_error_category=(
            str(item.get("expect_error_category"))
            if item.get("expect_error_category") is not None
            else None
        ),
        expect_hints_contains=[
            str(text) for text in _normalize_list(item.get("expect_hints_contains"))
        ],
        expect_json_fields=[str(text) for text in _normalize_list(item.get("expect_json_fields"))],
    )
