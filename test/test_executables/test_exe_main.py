...
# --- Import all configurations from the new config file ---
...
def setup_environment():
    """Validates paths, copies executables, and cleans the environment."""
...
def main():
    ...
    modules = [
        ...
        QueryTester(shared_counter, 3, 
                    ...
                    daily_query_date=config.DAILY_QUERY_DATE, 
                    monthly_query_month=config.MONTHLY_QUERY_MONTH, 
                    period_query_days=config.PERIOD_QUERY_DAYS,
                    **common_args),
        ExportTester(shared_counter, 4, 
                     generated_db_file_name=config.GENERATED_DB_FILE_NAME,
                     is_bulk_mode=config.EXPORT_MODE_IS_BULK,
                     specific_date=config.SPECIFIC_EXPORT_DATE,
                     specific_month=config.SPECIFIC_EXPORT_MONTH,
