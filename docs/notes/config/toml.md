...
- Agent 只需要调用一条 verify 命令：
  - `python tools/run.py verify --app tracer_core --build-dir build_fast --scope libs --scope cli --concise`
- 这条命令会自动完成：
  1. 触发 build（不在 CMake 中写死 config 路径）。
  2. build 阶段自动调用 `tools/platform_config/run.py`，生成并同步 Windows 侧配置。
...
### Android
- `tools/run.py` 执行 Android build 时，会自动调用 `tools/platform_config/run.py`。
