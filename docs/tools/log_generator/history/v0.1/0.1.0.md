...
### Architecture
* 端口接口上移至 `core/ports`，`Application/Workflow` 仅依赖接口。
* 依赖注入下沉至 `main` 作为组合根。
* 提取 `application` 应用层（编排/用例），并明确 `domain` 与 `infrastructure` 边界。
* CLI 拆分为 `framework` 与 `commands`，职责更清晰。

### Business
* 移除每年 1 月首日写入上一年 12 月 31 日的逻辑。
* 输出目录由 `Date` 更名为 `dates`。
* 新增 `--version` 命令，输出版本信息（基于 `version.hpp`）。

### Build
* 修复 `Sources.cmake` 的源文件列表与大小写，确保 CMake 正确生成。
* 同步 CMake 源文件路径到新的分层目录结构。
...
* 
**优化性能报告 (Performance Insightsing)**: 修改了控制台输出的统计文案，消除歧义 。
...
* `InsightsHandler`: 统一管理性能计时与最终报告输出。


* **上下文管理**: 引入 `AppContext` 数据结构，实现了配置数据在各处理器间的显式传递，消除了隐式依赖。
* **依赖优化 (Data-Behavior Separation)**: 新增 `common/ConfigTypes.hpp` 存放纯数据结构 (DTOs)。此举使 `CommandLineParser` 和 `PerformanceInsightser` 等组件不再依赖庞大的配置解析库 (toml++)，显著净化了模块依赖。
* **目录结构规范化**: `src/core` 内部细分为 `config/`, `workflow/`, `insights/` 子模块，实现了物理层面的关注点分离。
