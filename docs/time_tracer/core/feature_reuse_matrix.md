...
| 单次调用导入 `.zip`（解密后按月替换入库）能力基元（`exchange import`） | `done` | `done` | `not_started` | Windows 与 Android 共用密码保护 ZIP exchange 导入链路。 | CLI exchange tests: `tools/suites/tracer_windows_rust_cli/tests/commands_exchange.toml`; Android 导入说明：`docs/time_tracer/clients/android_ui/features.md` | — |
