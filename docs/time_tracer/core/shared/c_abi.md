...
14. `tracer_core_runtime_config_json` contract:
   ...
   - `apply_alias_hierarchy_operation`
     ...
     - optional operation fields are `target_path`, `destination_path`, `canonical_key`, `new_name`, `target_alias`, `old_alias`, and `aliases`; their required combination is determined by `kind`
   ...
   - `rewrite_alias_hierarchy_document`
   ...
   - `create_alias_hierarchy_document`
     - request: non-empty `parent`
     - response: minimal canonical `toml_content`
   - `render_alias_hierarchy_text`
