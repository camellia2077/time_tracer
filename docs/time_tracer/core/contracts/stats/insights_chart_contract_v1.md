# Insights Chart Contract v1

## 元信息
1. 版本：`v1`
2. 生效日期：`2026-02-23`
3. 适用 action：`insights-chart`
4. 适用输出模式：`output_mode=semantic_json`

## 目的
1. 定义 `insights-chart` 的跨端统一数据契约（Core -> Android -> Windows）。
2. 明确 Core 负责“可直接渲染的数据与统计口径”，端侧只做渲染。
3. 作为 `insights-chart` 字段约束的专用入口，避免字段语义漂移。
...
## `insights-composition` 展示语义

`insights-composition` 返回的加权 `tree` 之外，还必须提供以下展示字段：
...
## 代码落点
1. Core 映射入口：
   - `libs/tracer_core/src/infra/query/data/repository/query_runtime_service_insights_mapping.cpp`
2. Android 解析入口：
