# Insights Composition Contract v1

## 元信息

1. 版本：`v1`
2. 生效日期：`2026-07-11`
3. 适用 action：`insights-composition`
...
6. `average_duration_per_occurrence_seconds` (`int`, 必填，非负)：`duration_seconds / occurrence_count`，`occurrence_count=0` 时为 `0`。
7. `average_occurrence_ratio` (`number`, 必填，`0..1`)：当前节点记录次数除以当前层级所有节点记录次数；当前层级没有记录时为 `0`。
8. `children` (`object[]`, 必填，递归使用同一结构)
...
1. Core 输出：`libs/tracer_core/src/infra/query/data/repository/query_runtime_service_insights_mapping.cpp`
2. Android 解析：`apps/android/runtime/src/main/java/com/example/tracer/runtime/NativeRuntimeQueryParsing.kt`
3. Android 饼图下钻：`apps/android/feature-insights/src/main/java/com/example/tracer/ui/screen/InsightsCompositionVisualizationSection.kt`
