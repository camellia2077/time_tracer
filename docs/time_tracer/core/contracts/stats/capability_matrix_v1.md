...
| `insights-chart` | `lookback_days` 或 `from/to` + 可选 `root` | `stats/insights_chart_stats_calculator.*` | `series`, `total_duration_seconds`, `average_duration_seconds`, `active_days`, `range_days` | 缺 root/无数据时 `series=[]` 且统计值为 0 |
