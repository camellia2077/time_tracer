...
## 边界说明
1. 统计公式只允许存在于 `libs/tracer_core/src/infrastructure/query/data/stats/`。
2. 时间范围解析与 action 编排只允许存在于 `libs/tracer_core/src/infrastructure/query/data/orchestrators/`。
3. 文本与语义 JSON 渲染只允许存在于 `libs/tracer_core/src/infrastructure/query/data/renderers/`。
