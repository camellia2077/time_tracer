...
输入：
- 年/月/日+事件行文本（例如 `y2021`、`d0101`、`0827英语单词`、`0900-1030概率统计`）
...
源码定位：
- `libs/tracer_core/src/application/parser/text_parser.cpp`：`TextParser::Parse(...)`
- `libs/tracer_core/src/application/parser/text_parser.cpp`：`TextParser::ParseLine(...)`、`TextParser::ExtractRemark(...)`
- `libs/tracer_core/src/application/parser/text_parser.cpp`：`TextParser::IsYearMarker(...)`、`TextParser::IsNewDayMarker(...)`
...
- `libs/tracer_core/src/application/pipeline/steps/pipeline_stages.cpp`：`ConverterStep::Execute(...)`
- `libs/tracer_core/src/domain/logic/converter/convert/core/converter_core.cpp`：`LogLinker::LinkLogs(...)`
