...
1. 一天即使只有 1 条 authored event，只要它本身是完整区间事件，也可能已经是“可统计且自洽”的记录
2. 因此，`authored event` 条数不能再单独决定 completeness
...
4. 在未来支持区间事件后，新增区间也不会与已有已记录区间重叠
...
1. 一天只记录少量显式区间是合法的
2. 两个已记录区间之间存在未记录时间缺口是合法的
3. 缺口表示作者不想记录或暂不记录，而不是系统错误
...
2. `listWakeKeywords()` 只表示 `aliases/_system.toml` 中 `sleep_inference.wake_keywords`
...
> completeness 取决于时间区间是否已经自洽，而不仅取决于 authored event 的数量。
