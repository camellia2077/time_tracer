...
Domain 层的所有实体都存放在 `libs/tracer_core/src/domain/model/` 目录下。

### 1.1 `DailyLog` (日记录实体)
**文件**: `libs/tracer_core/src/domain/model/daily_log.hpp`
...
### 1.3 `ActivityStats` (查询期统计快照)
**文件**: `libs/tracer_core/src/domain/model/time_data_models.hpp`

这是查询/报表阶段使用的统计面板。在 C++ 结构体中，它被扁平化为各项具体指标（如 `study_time`, `sleep_night_time`, `anaerobic_time`），用于从活动事实数据即时聚合，而不再在解析期直接落入 SQLite。
...
**核心逻辑在**：`libs/tracer_core/src/domain/logic/converter/convert/core/converter_core.cpp`
...
确保引擎数据的高度纯净，这就是 `validator` 存在的意义。只有被它放行的时间数据才被允许向后续传递。

**核心引擎文件**: `libs/tracer_core/src/domain/logic/validator/structure/structure_validator.cpp`
