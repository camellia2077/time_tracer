...
其目录位于：`libs/tracer_core/src/application/`
...
*   **入向解包 (`TextParser`)**：
    从充满不可控特性的文本文件中剥离有用信息。利用字符比对剥离出前四位是时钟、识别 `//` 备注项及物理续行、甚至负责校验 `25:99` 这种非法人造时间并抛掷异常 (`Diagnostic`)。
*   **出向压榨 (`MemoryParser`)**：
...
Time Tracer 能完美抽离核心逻辑复用于 Android 和 Windows 的秘密武器，当前主要收敛在 capability 对齐后的 `application/ports/<capability>/**` 子目录中；只有宿主运行时 / shell 组合桥语义保留在 `application/runtime_bridge/**`。
...
举例：
在 `application/ports/**` 中你会看到按 capability 分组的纯虚基类：
```cpp
...
- **监控契约**：
  ...
  (当架构深层捕捉到异常时，上报问题，由外部宿主决定是打印控制台红字，还是弹出一个 Toast。前者是 capability port，后者是宿主 runtime bridge，不应混同为 capability-owned port。)。
