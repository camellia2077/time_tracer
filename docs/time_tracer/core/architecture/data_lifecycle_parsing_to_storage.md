...
关于 day bucket、`Wake Anchor`、自动补出的跨天睡眠 activity、以及为什么单日总时长允许超过 `24h`，请配合阅读：
...
3. **主键与标记补全 (`DayStats`)**：
   它会为 `0102` 的各个活动补齐 `logical_id`、起止时间戳、跨午夜后的精确时长，并更新当天的 `hasStudyActivity` / `hasExerciseActivity` / `hasWakeAnchor` 等日级标记。学习、娱乐、洗漱等统计时长不再在这个阶段预聚合。
...
1. **洗涤与结构化隔离**：
   ...
   - 诸如 `0102` 头部记录的每日评价 `// alpha` 及格言，被直接拍扁成了 `DayData` 结构中的 `remark` 字段（用真实换行符连接）。
...
- 报表中的 `Total Time Recorded` 是 activity durations sum
- 它不是“首个活动开始时间到最后活动结束时间”的跨度
- 在当前业务设计中，它可以合法地大于 `24h`
