...
Presentation settings such as the category color are owned by Android and
stored in DataStore; they are not part of the Core-owned TOML document.
...
The only permitted top-level fields are `parent` and `canonical`.
