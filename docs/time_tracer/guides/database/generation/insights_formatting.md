# 报告格式化引擎 (Insights Formatting)

本文描述 Time Tracer 是如何将已经在内存中聚合好的 `InsightsData`（包含时长的各项统计和已建好的项目树）拼接转化为具体的渲染输出文件（如 Markdown、LaTeX、Typst 等）的。

权威代码入口位于 `libs/tracer_core/src/infra/insights/` 目录。

## 1. 核心架构与职责分层

在数据被查询和聚合后，它们会以强类型 DTO（Data Transfer Object）的形式存在，例如 `DailyInsightsData` 或 `MonthlyInsightsData`。
格式化引擎的任务，就是把这些 DTO 转化为最终的字符串文本流。这里遵循了严格的 **“两段式解耦”** 设计：

### 1.1 `InsightsDtoFormatter` (总调度枢纽)
- **文件**: `insights_dto_formatter.hpp`
- **职责**: 它并不亲自动手拼字符串！它内部持有一个 `FormatterRegistry` 注册表缓存（支持缓存各类语言的 Formatter 实例）。
- 当外部（如终端命令）想要导出一份 Markdown 报告时，它会拿着 `DailyInsightsData` 去缓存库里寻找对应的 `IInsightsFormatter` 实例，并将数据全权委托给该实例去执行 `FormatInsightsFromView(data)`。

### 1.2 `IInsightsExportWriter` (物理 IO 输出器)
- **文件**: `insights_dto_export_writer.hpp`
...
`insights/markdown/en/`、`insights/markdown/zh/` 和 `insights/markdown/ja/`。
配置通过 `insights.markdown.root` 指定根目录，通过 `default_locale` 指定默认语言，
...
不仅仅只有 Markdown。基于这套严谨的接口规约，Time Tracer 开发了丰富的输出变种，所有实现皆放置在 `libs/tracer_core/src/infra/insights/<维度>/formatters/` 下。
...
在最核心的字符串生成部分，依靠极简的原生内存拼接与多态的 `Formatter` 策略模式，确保了哪怕未来需要增加 HTML 报告格式，也仅仅只需新写一个实现 `IInsightsFormatter` 的类，核心架构无需任何改动。
