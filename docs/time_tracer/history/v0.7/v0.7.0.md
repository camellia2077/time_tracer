...
* 新增六类固定样本（`day/month/week/year/recent/range`）采样与对账链路，落地 `scripts/toolchain/quality_gates/reporting/collect_report_markdown_cases.py` 与 `test/golden/report_markdown/v1/`。
* 新增默认 `verify` 门禁链路，接入 `scripts/toolchain/quality_gates/reporting/report_consistency_audit.py` 与 `scripts/toolchain/quality_gates/reporting/report_markdown_render_snapshot_check.py`。
