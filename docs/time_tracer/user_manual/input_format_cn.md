...
- **日期行**: `MMDD` 四位数字。
  - **首日必须为当月 1 号**（例如 `0101`）。
- **备注行**: 统一以 `//` 开头。
  - 日期块中的第一条活动之前属于日备注；连续备注行按物理换行合并。
  - 第一条活动之后属于最近一条活动备注，直到下一条活动或日期行开始。
- **事件行**:
...
事件行可以附带行内备注，唯一分隔符是 `//`。活动备注可以使用后续物理行，
每个后续备注行也以 `//` 开头；空行仍会被忽略。TXT 中写入的两个字符 `\n`
按普通文本保存，不会被解码成换行。
