...
3. `crypto decrypt` 的成功摘要应表达“事务式完整导入”结果，而不是旧的解包目录语义。
4. `crypto decrypt` 成功摘要至少应稳定包含：
   - `Updated active text root`
   - `Replaced months`
   - `Preserved local months`
   - `Rebuilt months`
   - `Database rebuilt`
5. `licenses` 仅保留摘要与 `--full` 详细文本。

## 6. 变更检查清单

1. 本地跑：`python tools/run.py verify --app tracer_core --build-dir build_fast --concise`
