...
| `tertiary` | Secondary analytical emphasis | Timeline duration text, chart reference lines, and selected chart bars/points |
...
- Keep the selected cell's palette-derived duration fill unchanged.
- For Light/Dark Mode Themes, draw a three-dp outer outline whose black or white
  color maximizes contrast with the resolved `surfaceContainerLow` chart-card
  surface. Draw a one-dp inner outline whose black or white color contrasts
  with the cell fill. This covers both surface-style variants in each mode.
- For Fixed Appearance Themes, preserve the existing two-dp single inner
  outline chosen from the cell fill; do not apply surface adaptation.
- When the selected date changes, the cell scales from 92% to its full size
  while the outline fades in over 200 milliseconds.
- Keep unselected cells at their original opacity. Dimming them would change
  the perceived intensity of the duration buckets.
- Selection outlines are independent of `tertiary`, because user-selectable
  heatmap palettes and surface styles cannot guarantee sufficient contrast with
  a fixed theme accent.
