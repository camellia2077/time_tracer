...
- `apps/android/feature-data/src/main/java/com/example/tracer/ui/viewmodel/DataViewModel.kt`
...
### 目标
- Android 与 CLI 的正式数据交换统一收口为“完整交换包导出”。
- 用户可见导出不允许只导 TXT，也不允许只导配置 TOML。
...
- 包内至少包含：
  ...
  - `manifest.toml`
- Android Data 页只保留 `Export Complete Exchange Package` 入口。
- Config 页不再提供独立 config bundle import/export。

### 进度语义
- Android 导出进度采用阶段语义，而不是“正在导出某个 TXT”：
  1. 收集记录文本
  2. 收集解析配置
  3. 生成交换说明
  4. 打包并加密导出
  5. 写出目标文件
- 第 1 到 3 阶段由 Android UI 包装显示。
- 第 4 到 5 阶段沿用 core crypto progress，并映射到完整交换包语义。

### 导出结果摘要
- 导出完成摘要固定展示完整交换包内容：
  - `payload_txt_count`
  - `converter_toml_count`
  - `manifest_count`
- 当前 Android runtime / host 导出结果会显式返回：
  - `converter_file_count`
  - `manifest_included`

### 代码定位（Kotlin/Native）
- `apps/android/app/src/main/java/com/example/tracer/ui/screen/tracer/TracerScreenExports.kt`
  - Data 页面完整交换包导出入口、目录选择与口令弹窗。
- `apps/android/app/src/main/java/com/example/tracer/ui/screen/tracer/TracerScreenExportTracer.kt`
  - TXT staging、阶段进度包装、`.tracer` 写出与摘要构建。
- `apps/android/runtime/src/main/java/com/example/tracer/runtime/services/RuntimeTracerExchangeService.kt`
  - Android runtime export/import/inspect 结果映射。
- `apps/tracer_core_shell/api/android_jni/native_bridge_crypto_calls.cpp`
  - Android JNI export content JSON 构造。
- `apps/tracer_core_shell/api/c_api/tracer_core_c_api_crypto.cpp`
  - CLI / C API 完整交换包导出成功文本。
- `libs/tracer_core/src/infrastructure/crypto/tracer_exchange_service.cpp`
  - tracer exchange package 组装、manifest 写入与导出结果元数据。

## Data：完整交换包导入

### 目标
- Android `Import Single TRACER` 统一走完整交换包事务导入。
- 导入语义不再是“先解包 payload 再逐个 ingest”，而是由 runtime 一次性完成 config 应用、TXT 校验、本地月份更新与数据库重建。

### 导入契约
- 仅支持当前 `tracer exchange package v3`。
- payload 路径必须严格为：
  - `payload/<year>/YYYY-MM.txt`
- 文件名与内容头 `yYYYY + mMM` 必须完全一致。
- 导入时：
  - 包内月份覆盖本地同月 TXT
  - 包外本地月份继续保留
  - 但全部有效 TXT 都必须在新 `converter TOML` 下重新完成结构校验和逻辑校验
  - 校验通过后才会更新本地受管 TXT，并全量重建数据库

### 失败语义
- 任一步失败都回滚：
  - active converter config
  - 已替换的本地月文件
  - 数据库状态
- runtime 可以保留 transaction work root 作为失败调试现场，但 Android UI 不再把它当作正式导入产物目录。

### 代码定位（Kotlin/Native）
- `apps/android/app/src/main/java/com/example/tracer/ui/screen/tracer/TracerScreenImports.kt`
  - Data 页面单文件 `TRACER` 选择、口令输入与事务式导入入口。
- `apps/android/runtime/src/main/java/com/example/tracer/runtime/services/RuntimeTracerExchangeService.kt`
  - Android runtime import 结果字段映射与失败调试目录透传。
- `apps/tracer_core_shell/api/android_jni/native_bridge_crypto_calls.cpp`
  - Android JNI 事务式导入 request 组装与结果 JSON 投影。
- `libs/tracer_core/src/infrastructure/crypto/tracer_exchange_service.cpp`
  - `v3` package 校验、有效 TXT 视图构建、事务回滚与全量数据库重建。

## Config：本机 TOML 浏览与编辑

### 目标
- `Config` 页面只负责查看和编辑本机 runtime config。
- 用户修改的是 Android 本机运行时快照，不再提供独立 config bundle 导入/导出交换入口。

### 行为约束
- 可浏览 `converter` 与 `reports` 两类 TOML 文件。
- 可在编辑器中修改当前文件并保存到本机 runtime config。
- 可复制 diagnostics payload 供排障使用。
- 不支持从目录导入 Android config bundle，也不支持导出一组 TOML 作为交换包。

### 代码定位（Kotlin）
- `apps/android/app/src/main/java/com/example/tracer/ui/screen/ConfigScreen.kt`
  - Config 页面组合与 About/Diagnostics 入口。
- `apps/android/runtime/src/main/java/com/example/tracer/runtime/RuntimeEnvironment.kt`
  - 初始化阶段配置文件“仅补齐不覆盖”策略。
- `apps/android/runtime/src/main/java/com/example/tracer/runtime/ConfigTomlStorage.kt`
  - 本机 runtime config TOML 的列举、读取与保存。
- `apps/android/app/src/main/java/com/example/tracer/ui/viewmodel/ConfigViewModel.kt`
  - Config 页面文件列表、当前文件与保存动作状态管理。
...
- `apps/android/feature-record/src/main/java/com/example/tracer/ui/screen/TxtEditorContentCard.kt`
  - `ALL/DAY` 切换、`DAY Content` 文本框行为、`Save & Sync` 按钮可用条件。
- `apps/android/feature-record/src/main/java/com/example/tracer/ui/viewmodel/RecordUseCases.kt`
...
- `apps/android/feature-report/src/main/java/com/example/tracer/ui/screen/ReportChartResultContent.kt`
  - 图表模式页面编排入口。
- `apps/android/feature-report/src/main/java/com/example/tracer/ui/screen/ReportChartParameterSection.kt`
...
### 多语言
- 新增图表模式相关文案已同步到：
  - `apps/android/feature-report/src/main/res/values/strings.xml`
  - `apps/android/feature-report/src/main/res/values-zh/strings.xml`
  - `apps/android/feature-report/src/main/res/values-ja/strings.xml`
