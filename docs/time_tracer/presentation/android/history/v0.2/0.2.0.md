...
* 重构 **QueryReportScreen**：将 `apps/android/feature-report/src/main/java/com/example/tracer/ui/screen/QueryReportScreen.kt` 拆分为 `ReportCard`、`StatsCard`、`TreeCard`、`SegmentedInputs`，保持原有交互与输出不变。
* 重构 **QueryReportViewModel**：将 `apps/android/feature-report/src/main/java/com/example/tracer/ui/viewmodel/QueryReportViewModel.kt` 拆分为“报表触发”“分析触发”“参数校验”三层，保持状态字段与对外接口不变。
* 重构 **TracerScreen**：将 `apps/android/app/src/main/java/com/example/tracer/ui/screen/TracerScreen.kt` 拆分为“底部导航壳”“Tab 路由层”“结果展示层”，保持页面路由与状态展示不变。
* 重构 **ConfigScreen**：将 `apps/android/app/src/main/java/com/example/tracer/ui/screen/ConfigScreen.kt` 拆分为“分类切换”“文件列表”“编辑器”“主题设置”四块，保持配置编辑流程不变。
* 重构 **NativeRuntimeController**：将 `apps/android/runtime/src/main/java/com/example/tracer/runtime/NativeRuntimeController.kt` 按 `init / ingest / query / report / record / storage` 六块进行职责收敛，保持 `RuntimeGateway` 行为与返回结构不变。
* 重构 **LiveRawRecordStore**：将 `apps/android/runtime/src/main/java/com/example/tracer/runtime/LiveRawRecordStore.kt` 拆分为“输入规范化”“记录解析”“落盘写入”三块，保持 TXT 写入结果与错误语义不变。
...
* **重构基线与 ADR 补齐**：在 `temp/tabs_refactor.md` 补充 Phase 0 行为矩阵与完成标记；新增 ADR：`apps/android/docs/adr/0001-typed-tab-and-registry.md`。
