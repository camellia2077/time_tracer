...
- 结构文档补齐：
  ...
  - `apps/android/agent.md` 新增 Structure 文档链接，保证入口可发现。
...
### Docs Cleanup（路径收敛）
- Android 文档从 `apps/android` 收敛到 `docs/time_tracer/clients/android_ui/specs/`：
...
- `apps/android/agent.md` 链接已更新到新路径。
...
### 验证
- `python tools/run.py build --app tracer_android --profile fast` 通过（`BUILD SUCCESSFUL`）。
...
- `python tools/run.py post-change --app tracer_android` 通过：
