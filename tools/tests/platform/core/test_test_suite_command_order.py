...
class TestSuiteCommandOrder(TestCase):
    def test_insights_stages_preserve_include_order_before_exchange(self):
        ...
        self.assertIn("insights", stage_indexes)
        self.assertIn("insights-export", stage_indexes)
        self.assertIn("exchange", stage_indexes)
        self.assertLess(
            stage_indexes["insights"],
            stage_indexes["exchange"],
            "insights stage should follow tests.toml include order and run before exchange",
        )
        self.assertLess(
            stage_indexes["insights-export"],
            stage_indexes["exchange"],
            "insights-export stage should follow tests.toml include order and run before exchange",
        )
