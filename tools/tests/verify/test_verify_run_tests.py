...
class TestVerifyRunTests(VerifyCommandTestBase):
    ...
    def test_run_tests_tracer_android_release_device_uses_profile_config(self):
        with (
            patch(
                "tools.toolchain.commands.cmd_quality.verify.run_command", return_value=0
            ) as mocked_run,
            ...
            result = self.run_tests_silently(
                app_name="tracer_android",
                build_dir_name="build_fast",
                profile_name="android_release_device",
                concise=True,
            )
        ...
        called_cmd = mocked_run.call_args.args[0]
        self.assertIn("--config", called_cmd)
        self.assertIn("config_android_release_device.toml", called_cmd)
    ...
    def test_run_tests_shell_aggregate_uses_shell_suite_config(self):
        with patch(
            "tools.toolchain.commands.cmd_quality.verify.run_command", return_value=0
        ) as mocked_run:
            result = self.run_tests_silently(
        ...
        self.assertIsNotNone(called_cmd)
        self.assertIn("--config", called_cmd)
        self.assertIn("config_shell_aggregate.toml", called_cmd)
