...
class TestVerifyMarkdownGateRunner(TestCase):
    def test_cap_query_skips_insights_markdown_gates(self):
        mocked_run = Mock(return_value=0)

        result = run_insights_markdown_gates(
            repo_root=REPO_ROOT,
            setup_env_fn=lambda: {},
    ...
    def test_cap_insights_runs_insights_markdown_gates(self):
        mocked_run = Mock(return_value=0)
        ...
        result = run_insights_markdown_gates(
            ...
            app_name="tracer_core_shell",
            build_dir_name="build_fast",
            profile_name="cap_insights",
        )

        self.assertEqual(result, 0)
        called_commands = [" ".join(call.args[0]) for call in mocked_run.call_args_list]
        self.assertTrue(
            any("collect_insights_markdown_cases.py" in command for command in called_commands)
        )

    def test_shell_aggregate_skips_insights_markdown_gates(self):
        mocked_run = Mock(return_value=0)
