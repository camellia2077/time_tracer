...
class ValidateCommandTestBase(TestCase):
    _BUILD_PATCH = "tools.toolchain.commands.cmd_validate.command.BuildCommand.build"
    _CONFIGURE_PATCH = "tools.toolchain.commands.cmd_validate.command.BuildCommand.configure"
    ...
    def _write_plan_lines(self, repo_root: Path, lines: list[str]) -> Path:
        return self._write_plan(repo_root, "\n".join(lines) + "\n")

    def _execute_silently(self, command: ValidateCommand, **kwargs) -> int:
        with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
            return command.execute(**kwargs)

    def _load_summary(self, repo_root: Path, run_name: str) -> dict:
        summary_path = repo_root / "out" / "validate" / run_name / "summary.json"
        return json.loads(summary_path.read_text(encoding="utf-8"))


class TestValidateCommandBuildTracks(ValidateCommandTestBase):
    def test_execute_writes_summary_for_successful_tracks(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = Path(temp_dir)
            self._write_minimal_repo(repo_root)
            command = ValidateCommand(Context(repo_root))
            plan_path = self._write_plan_lines(
                repo_root,
                [
                    "[run]",
                    'name = "import_batch01"',
            ...
            with patch(self._BUILD_PATCH, new=fake_build):
            ...
            payload = self._load_summary(repo_root, "import_batch01")
    ...
    def test_execute_uses_plan_scope_when_cli_scope_missing(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result, 0)
            payload = self._load_summary(repo_root, "import_batch_scope_default")
            self.assertEqual(payload["scope_paths"], ["libs/tracer_core/src/application/query/tree"])

    def test_execute_failure_flow_respects_continue_on_failure(self):
        scenarios = [
            {
                "run_name": "import_batch02",
                "continue_on_failure": None,
                "expected_rc": 5,
                "expected_calls": ["build_one", "build_two"],
                "expected_track_count": 2,
                "first_error": "fatal: first track failed",
            },
            {
                "run_name": "import_batch03",
                "continue_on_failure": False,
                "expected_rc": 9,
                "expected_calls": ["build_one"],
                "expected_track_count": 1,
                "first_error": "fatal: stop here",
            },
        ]

        for scenario in scenarios:
            with self.subTest(run_name=scenario["run_name"]):
                with tempfile.TemporaryDirectory() as temp_dir:
                    repo_root = Path(temp_dir)
                    self._write_minimal_repo(repo_root)
                    command = ValidateCommand(Context(repo_root))
                    run_lines = [
                        "[run]",
                        f'name = "{scenario["run_name"]}"',
                    ]
                    if scenario["continue_on_failure"] is False:
                        run_lines.append("continue_on_failure = false")
                    plan_path = self._write_plan_lines(
                        repo_root,
                        run_lines
                        + [
                    ...
                    def fake_build(self, **kwargs):
                        ...
                        if kwargs["build_dir_name"] == "build_one":
                            print(scenario["first_error"])
                            return scenario["expected_rc"]
                    ...
                    self.assertEqual(result, scenario["expected_rc"])
                    self.assertEqual(calls, scenario["expected_calls"])
                    payload = self._load_summary(repo_root, scenario["run_name"])
                    self.assertFalse(payload["success"])
                    self.assertEqual(len(payload["tracks"]), scenario["expected_track_count"])
                    self.assertEqual(payload["failures"][0]["step"], "build")
                    self.assertIn(scenario["first_error"], payload["failures"][0]["key_error_lines"])
    ...
    def test_execute_build_and_configure_tracks_pass_concise_to_build_command(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch(self._CONFIGURE_PATCH, new=fake_configure),
                patch(self._BUILD_PATCH, new=fake_build),
            ):
                result = self._execute_silently(
                    command,
                    plan_path=str(plan_path),
    ...
    def test_execute_configure_and_build_tracks_run_command_entry_smoke(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            payload = self._load_summary(repo_root, "import_batch06")
            self.assertTrue(payload["success"])
...
class TestValidateCommandVerifyTracks(ValidateCommandTestBase):
