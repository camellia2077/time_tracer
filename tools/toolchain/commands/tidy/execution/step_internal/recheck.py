...
from ....clang.tidy import config as clang_tidy_config, diagnostics as log_parser
...
def run_task_recheck(
    ...
    *,
    ...
    log_path = tidy_layout.automation_dir / f"{parsed.cluster_id}_task_{parsed.task_id}_recheck.log"
    if ensure_ret != 0:
...
def rewrite_task_artifacts_from_recheck(
    *,
    ...
    refreshed_record = finalize_task_record(
        ...
        cluster_id=task_ctx.parsed_task.cluster_id,
        scan_id=task_ctx.parsed_task.scan_id,
    ...
    existing_artifacts = {artifact.suffix.lower() for artifact in task_view_paths(json_path)}
