...
def resolve_explicit_task_json_path(
    ...
    *,
    ...
    if not TASK_FILE_PATTERN.match(candidate.name):
        raise ValueError("`--task-log` must point to a canonical task_*.json record.")
...
def _parse_task_artifact_layout(task_json_path: Path) -> tuple[str, str, Path]:
    cluster_dir = task_json_path.parent
    clusters_dir = cluster_dir.parent
    tasks_dir = clusters_dir.parent
    if clusters_dir.name != "clusters" or tasks_dir.name != "tasks":
        raise ValueError(
            "task path must live under out/tidy/<app>/<workspace>/tasks/clusters/<cluster>/task_xxx.json"
        )
