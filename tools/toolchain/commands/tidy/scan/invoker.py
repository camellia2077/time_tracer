...
def resolve_effective_tidy_jobs(
    ...
    *,
    ...
    if mode == "full":
        ...
        # final tidy runs stay bounded after stdout goes quiet.
        return max(1, min(_AUTO_FULL_TIDY_JOB_CAP, max(1, cpu_count // 3)))

    # Task helpers work on smaller scopes than full tidy, so they can use
    # a slightly wider bounded fan-out without reintroducing unbounded builds.
    return max(1, min(_AUTO_TASK_JOB_CAP, max(1, cpu_count // 2)))
...
def resolve_tidy_paths(
    ...
    return {
        ...
        "structured_results_dir": build_dir / "structured_tidy_results",
