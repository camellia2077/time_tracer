        ...
        while True:
            ...
            if task_kind == "rename_only":
                if not rename_done:
                    ...
                    ret = rename_cmd.plan(app_name, tidy_build_dir_name=resolved_tidy_build_dir)
                    if ret != 0:
                        return ret
                    ret = rename_cmd.apply(
                        app_name,
                        strict=True,
                        tidy_build_dir_name=resolved_tidy_build_dir,
                    )
                    if ret != 0:
                        return ret
                    ret = rename_cmd.audit(
                        app_name,
                        strict=True,
                        tidy_build_dir_name=resolved_tidy_build_dir,
                    )
