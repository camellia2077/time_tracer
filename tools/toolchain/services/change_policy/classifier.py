...
def is_cmake_related(path: str) -> bool:
    ...
    if lower.endswith(".cmake"):
        return True
    if posix_path.parts == ("tools", "toolchain", "config.toml"):
        return True
    if lower.startswith("tools/toolchain/config/") and lower.endswith(".toml"):
        return True
...
def is_script_file(path: str) -> bool:
    return path.lower().startswith(("scripts/", "tools/"))
