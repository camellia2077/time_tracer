...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect fixed markdown insights cases for golden/audit checks."
    )
    parser.add_argument(
        "--export-root",
        required=True,
        help="Export markdown root (for example out/test/.../insights/markdown).",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
...
def collect_file_cases(
    ...
    for file_case in file_cases:
        ...
        if not source_path.is_file():
            raise FileNotFoundError(f"missing source insights: {source_path}")
...
def collect_query_range_case(
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(
            f"range insights render failed while collecting `{output_name}`: "
...
def collect_query_recent_case(
    ...
    if completed.returncode != 0:
        stderr_text = completed.stderr.decode("utf-8", errors="replace")
        raise RuntimeError(
            f"recent insights render failed while collecting `{output_name}`: "
            f"exit={completed.returncode}, stderr={stderr_text}"
        )
    content = normalize_crlf_to_lf(completed.stdout)
    if strict:
