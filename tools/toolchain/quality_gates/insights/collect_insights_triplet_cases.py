...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=("Collect fixed day/month/range insights cases for md/tex/typ snapshot checks.")
    )
    parser.add_argument(
        "--format",
        required=True,
        choices=sorted(FORMAT_SPECS.keys()),
        help="Insights format: md, tex, typ.",
    )
    parser.add_argument(
        "--export-root",
        required=True,
        help="Export root (for example out/test/.../insights/markdown).",
    )
    parser.add_argument(
        "--output-dir",
        required=True,
...
def collect_query_range_case(
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(
            f"range insights render failed while collecting `{case_name}`: "
