...
from ....commands.cmd_quality.insights_markdown_gate import InsightsMarkdownGateCommand
...
def register(parser: argparse.ArgumentParser, _defaults: ParserDefaults) -> None:
    ...
    parser.add_argument(
        "--refresh-golden",
        action="store_true",
        help="Sync current markdown cases into test/golden/insights_markdown/v1 before re-auditing.",
    )
    parser.add_argument(
        "--cases-config",
        default=None,
...
def run(args: argparse.Namespace, ctx: Context) -> int:
    build_dir_error = reject_unsupported_build_dir_override(
        ...
        command_name="insights-markdown-gate",
    )
    if build_dir_error != 0:
        return build_dir_error
    cmd = InsightsMarkdownGateCommand(ctx)
    return cmd.execute(
        app_name=args.app,
        build_dir_name=args.build_dir,
