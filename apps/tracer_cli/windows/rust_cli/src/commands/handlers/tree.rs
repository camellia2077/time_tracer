...
use crate::core::runtime::{CoreApi, TreeNode};
use crate::error::AppError;

pub struct TreeHandler;

impl CommandHandler<TreeArgs> for TreeHandler {
    fn handle(&self, args: TreeArgs, ctx: &CommandContext) -> Result<(), AppError> {
        let api = CoreApi::load()?;
        let (runtime, _cli_config) = api.bootstrap("tree", ctx)?;

        if args.roots {
            let response = runtime.run_tree(&json!({
                "list_roots": true,
                "root_pattern": "",
                "max_depth": -1
            }))?;
