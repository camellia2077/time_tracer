...
> - Windows CLI 默认图标设计源已经切换到 `bg_golden_vertical_padding.svg`
...
```powershell
python scripts/run.py build --app tracer_windows_rust_cli --profile release_bundle --build-dir build --windows-icon-svg "design/branding/exports/bg_golden_vertical_padding.svg"
```

### 3) 通过封装脚本指定 SVG

```powershell
bash apps/tracer_cli/windows/scripts/build_rust_from_windows_build.sh --windows-icon-svg "design/branding/exports/bg_golden_vertical_padding.svg"
```
