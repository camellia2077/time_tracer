...
// inside insights/chart_* helpers so `insights chart` stays in the insights family.
use std::env;
use std::fs;
...
fn run_chart_with_query_port(
    ...
    let semantic_payload = query_port.query_data("insights", ctx, &request)?;
...
mod tests {
    ...
    fn chart_handler_uses_chart_session_and_writes_html_output() {
        ...
        assert_eq!(recorded_commands, vec!["insights".to_string()]);
        assert_eq!(recorded_requests.len(), 1);
        assert_eq!(recorded_requests[0]["action"], "insights-chart");
        ...
        assert!(html.contains("Insights Chart (Line)"));
    ...
    #[test]
    fn chart_handler_routes_pie_to_insights_composition_without_root() {
        ...
        let recorded_requests = recorder.requests();
        assert_eq!(recorded_requests.len(), 1);
        assert_eq!(recorded_requests[0]["action"], "insights-composition");
