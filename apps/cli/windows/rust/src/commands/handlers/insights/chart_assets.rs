...
pub(crate) fn load_chart_assets(args: &ChartArgs) -> Result<ChartAssets, AppError> {
    ...
    let base_template =
        fs::read_to_string(assets_root.join("templates/insights_chart_base.html.tpl"))
...
fn resolve_assets_root() -> Result<PathBuf, AppError> {
    ...
    if let Ok(cwd) = env::current_dir() {
        ...
        while let Some(dir) = cursor {
            candidates.push(dir.join("apps/cli/windows/rust/runtime/assets"));
            cursor = dir.parent();
        }
    }
    ...
    candidates
        .into_iter()
        .find(|dir| dir.join("templates/insights_chart_base.html.tpl").exists())
