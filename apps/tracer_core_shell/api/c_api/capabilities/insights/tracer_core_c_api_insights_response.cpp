...
auto BuildInsightsTextResponse(const TextOutput& output) -> const char* {
  if (!output.ok) {
  ...
  envelope.insights_hash_sha256 = ComputeSha256Hex(output.content);
  if (output.insights_window_metadata.has_value()) {
    envelope.insights_window_metadata =
        tt_transport::InsightsWindowMetadataPayload{
            .has_records = output.insights_window_metadata->has_records,
            .matched_day_count =
                output.insights_window_metadata->matched_day_count,
            .matched_record_count =
                output.insights_window_metadata->matched_record_count,
            .start_date = output.insights_window_metadata->start_date,
            .end_date = output.insights_window_metadata->end_date,
            .requested_days = output.insights_window_metadata->requested_days,
        };
  }
