...
auto main() -> int {
  ...
  try {
    ...
    const fs::path kTempRoot =
        kRepoRoot / "test" / "output" / "tracer_core_c_api_pipeline";
...

    {
      ...
      RunPipelineChecks(api, runtime.Get(), kInputRoot);
    }

    std::cout << "[PASS] tracer_core_c_api_pipeline_tests\n";
    CloseLibrary(library);
    return 0;
  } catch (const std::exception& error) {
    std::cerr << "[FAIL] tracer_core_c_api_pipeline_tests: " << error.what()
              << '\n';
    return 1;
  }
}
