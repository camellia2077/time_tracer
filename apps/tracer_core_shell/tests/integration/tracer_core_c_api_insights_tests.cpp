...
void RunInsightsChecks(const CoreApiFns& api, TtCoreRuntimeHandle* runtime,
  ...
  const json kInsightsResponse = ParseResponse(
      api.runtime_insights(runtime, json{{"operation_kind", "query"},
                                         {"display_mode", "day"},
  ...
  const json kInsightsResponseAgain = ParseResponse(
      api.runtime_insights(runtime, json{{"operation_kind", "query"},
                                         {"display_mode", "day"},
                                         {"selection_kind", "single_day"},
                                         {"date", "2025-01-03"},
  ...
  Require(
      kInsightsResponseAgain.value("content", std::string{}) ==
  ...
  const json kStructuredInsightsResponse = ParseResponse(
      api.runtime_insights(runtime, json{{"operation_kind", "structured_query"},
                                         {"display_mode", "day"},
                                         {"selection_kind", "single_day"},
                                         {"date", "2025-01-03"}}
                                        .dump()
  ...
  Require(!kStructuredRecords.empty() &&
              kStructuredRecords.at(0).value("parent_color", std::string{}) ==
                  "#22C55E",
          "structured insights should expose the configured parent color per "
          "record");
  ...
  Require(
      kStructuredRangeStatuses.at(0).contains("occurrence_count") &&
  ...
  Require(kStructuredRangeActivityDays.at(0).value("date", std::string{}) ==
          ...
          "structured range activity day should keep its date and detailed "
          "records");
  ...
  Require(kRangeInsightsResponse.value("content", std::string{})
                  .find(kStructuredRangeStatuses.at(0)
                            .at("label")
                            .get<std::string>()) != std::string::npos,
  ...
  Require(
      kInsightsBatchHash.size() == kSha256HexLength,
      "baseline runtime insights batch should include 64-char insights hash");
  Require(
      kInsightsBatchHash != kInsightsHash ||
  ...
  const json kTargetsResponse = ParseResponse(
      api.runtime_insights(runtime, json{{"operation_kind", "targets"},
                                         {"display_mode", "month"}}
                                        .dump()
                                        .c_str()),
  ...
  Require(
      kTargetsResponse.contains("items") &&
          kTargetsResponse["items"].is_array(),
  ...
  Require(!kEmptyRangeResponse.contains("error_code") ||
          ...
          "empty range runtime insights should not expose "
  ...
  Require(
      kEmptyRangeResponse.value("start_date", std::string{}) == "2024-12-01",
      "empty range runtime insights should expose start_date");
  Require(kEmptyRangeResponse.value("end_date", std::string{}) == "2024-12-31",
  ...
  const json kMissingInsightsResponse = ParseResponse(
      api.runtime_insights(runtime, json{{"operation_kind", "query"},
                                         {"display_mode", "day"},
                                         {"selection_kind", "single_day"},
                                         {"date", "2024-12-31"},
  ...
  Require(kMissingInsightsResponse.value("error_code", std::string{}) ==
              "insights.target.not_found",
          "missing runtime insights target should expose "
          "insights.target.not_found");
  ...
  const json kMissingExportResponse = ParseResponse(
      api.runtime_insights(runtime, json{{"operation_kind", "export"},
                                         {"display_mode", "month"},
                                         {"export_scope", "single"},
                                         {"selection_kind", "date_range"},
                                         {"start_date", "2024-12-01"},
  ...
  Require(
      kMissingExportResponse.value("error_code", std::string{}) ==
          "insights.target.not_found",
  ...
  RequireOk(api.runtime_insights(runtime, json{{"operation_kind", "export"},
                                               {"display_mode", "month"},
                                               {"export_scope", "single"},
                                               {"selection_kind", "date_range"},
  ...
  Require(
      fs::exists(output_root / "markdown" / "month" / "2025-01.md"),
  ...
  const json kInvalidRecentInsightsResponse = ParseResponse(
      api.runtime_insights(runtime, json{{"operation_kind", "query"},
                                         {"display_mode", "recent"},
  ...
  Require(
      kInvalidRecentInsightsResponse.value("error_code", std::string{}) ==
          "runtime.generic_error",
      "invalid recent runtime insights should use generic runtime error code");
  Require(kInvalidRecentInsightsResponse.value("error_message", std::string{})
                  .find("Recent argument must be a positive integer.") !=
              std::string::npos,
          "invalid recent runtime insights should explain positive-integer "
          "requirement");

  const json kInvalidTargetsResponse = ParseResponse(
      api.runtime_insights(runtime, json{{"operation_kind", "targets"},
                                         {"display_mode", "quarter"}}
                                        .dump()
                                        .c_str()),
  ...
  Require(
      kInvalidTargetsResponse.value("error_code", std::string{}) ==
          "runtime.generic_error",
      "invalid runtime insights targets should use generic runtime error code");
  Require(
      kInvalidTargetsResponse.value("error_message", std::string{})
              .find("field `type` must be one of") != std::string::npos,
