...
void RunPipelineChecks(const CoreApiFns& api, TtCoreRuntimeHandle* runtime,
  ...
  const json kWrittenQuickAccess = ParseResponse(
      api.runtime_txt(runtime, json{{"action", "write_quick_access"},
  ...
  const json kClearedSyncStatus = ParseResponse(
      api.runtime_ingest_sync_status(runtime, json::object().dump().c_str()),
  ...
  const json kInvalidTimeOrderMode =
      ParseResponse(api.runtime_record_activity_atomically(
  ...
  const json kResolveDay = ParseResponse(
      api.runtime_txt(
          runtime,
          json{{"action", "resolve_day_block"},
               {"content", std::string("y2025\nm01\n\nd0101\n0900study\n\nd0102"
                                       "\n0656w\n0904无氧训练\n")},
               {"day_marker", "0102"},
  ...
  Require(
      kReplaceDay.value("updated_content", std::string{})
              .find("d0102\n1111new_line\n") != std::string::npos,
  ...
  const json kCanonicalActivityNames = ParseResponse(
      api.runtime_txt(runtime, json{{"action", "convert_activity_names"},
  ...
  const json kCanonicalReplacement = ParseResponse(
      api.runtime_txt(
          ...
          json{{"action", "replace_canonical_activity_names"},
               ...
               {"replacements",
                json::array({{{"old_canonical", "exercise_walk"},
  ...
  Require(
      kCanonicalReplacement.value("updated_content", std::string{}) ==
          "y2026\nm01\n\n0830exercise_cardio_walk // exercise_walk remark\n",
      "canonical replacement should preserve remarks and replace only event "
      "names");

  const json kAliasReplacement = ParseResponse(
      api.runtime_txt(runtime, json{{"action", "replace_alias_activity_names"},
                                    {"content",
                                     "y2026\nm01\n\n012431有氧\n012500exercise_"
                                     "cardio // keep canonical\n"},
                                    {"replacements",
                                     json::array({{{"old_alias", "有氧"},
                                                   {"new_alias", "有氧aa"}}})}}
                                   .dump()
                                   .c_str()),
  ...
  Require(kApplyActivityHierarchyOperation
  ...
  Require(
      kHierarchy.value("parent", std::string{}) == "exercise" &&
          std::ranges::any_of(kHierarchy.at("nodes"),
                              [](const auto& node) {
  ...
  const json kSetParentColor = ParseResponse(
      api.runtime_txt(
          runtime, json{{"action", "apply_activity_hierarchy_operation"},
                        {"toml_content", kActivityHierarchyToml},
                        {"operation",
                         {{"kind", "set_parent_color"}, {"color", "#FF0000"}}}}
                       .dump()
                       .c_str()),
  ...
  const json kClearParentColor = ParseResponse(
      api.runtime_txt(
          ...
          json{
              ...
              {"operation", {{"kind", "set_parent_color"}, {"color", nullptr}}}}
              .dump()
              .c_str()),
  ...
  Require(kRenameParent.value("updated_toml_content", std::string{})
  ...
  const json kDescribeActivityHierarchy = ParseResponse(
      api.runtime_txt(runtime, json{{"action", "describe_activity_hierarchy"},
                                    {"toml_content", kActivityHierarchyToml}}
                                   .dump()
                                   .c_str()),
      "describe activity hierarchy");
  Require(
      kDescribeActivityHierarchy.value("ok", false) &&
  ...
  Require(
      kRenderActivityHierarchyText.value("ok", false) &&
          kRenderActivityHierarchyText.value("content", std::string{})
                  .find("cardio — group_aliases: 有氧") != std::string::npos,
  ...
  Require(kCrossDocumentReplacements.size() == 1 &&
              ...
              kCrossDocumentReplacements[0].value("new_canonical",
                                                  std::string{}) == "meal_go",
  ...
  Require(
      kInvalidHierarchyOperation.value("error_code", std::string{}) ==
              "config.activity_hierarchy.failed" &&
          kInvalidHierarchyOperation.value("error_category", std::string{}) ==
              "config",
  ...
  Require(
      kDefaultMarker.value("normalized_day_marker", std::string{}) == "0228",
  ...
  const json kInvalidValidateLogic =
      ParseResponse(api.runtime_validate_logic(
                        runtime, json{{"input_path", input_root.string()},
  ...
  Require(kInvalidValidateLogic.value("error_code", std::string{}) ==
          ...
          "invalid validate logic date_check_mode should use generic runtime "
          "error code");
  Require(
      kInvalidValidateLogic.value("error_message", std::string{})
              .find("field `date_check_mode` must be one of") !=
          std::string::npos,
