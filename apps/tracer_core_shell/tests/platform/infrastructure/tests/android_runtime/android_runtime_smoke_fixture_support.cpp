...
auto BuildRuntimeFixture(std::string_view test_name, int& failures)
  ...
  fixture.config_toml_path =
      fixture.paths.test_root / "config" / "user" / "behavior.toml";
  ...
  try {
    ...
    std::filesystem::copy(repo_root / "config" / "program",
                          config_root / "program",
    ...
    std::filesystem::copy(repo_root / "test" / "data" / "activity_hierarchy",
