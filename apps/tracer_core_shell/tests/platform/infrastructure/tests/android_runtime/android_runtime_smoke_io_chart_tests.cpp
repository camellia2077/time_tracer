// infrastructure/tests/android_runtime/android_runtime_smoke_io_chart_tests.cpp
#include <iostream>
#include <string>
...
auto ProbeChartRange(const std::shared_ptr<ITracerCoreRuntime>& runtime_api,
  ...
  if (const auto chart_range_result = RunDataQueryOrRecordFailure(
    ...
    if (const auto payload = ParseJsonOrRecordFailure(
        ...
        payload.has_value()) {
      ValidateChartSeriesPayload(*payload, "insights-chart range", failures);
      const auto series_it = payload->find("series");
      if (series_it == payload->end() || !series_it->is_array() ||
          series_it->size() != 7U) {
        ++failures;
        std::cerr << "[FAIL] insights-chart range should return 7 points for "
                     "lookback_days=7.\n";
      }
      ...
      if (roots_it == payload->end() || !roots_it->is_array() ||
          roots_it->empty()) {
        ++failures;
        std::cerr << "[FAIL] insights-chart range roots should not be empty "
                     "after ingest.\n";
      } else {
        ...
        for (size_t index = 1; index < roots.size(); ++index) {
          if (roots[index - 1].is_string() && roots[index].is_string() &&
            ...
            ++failures;
            std::cerr << "[FAIL] insights-chart roots should be sorted "
                         "ascending.\n";
            break;
          }
        }
...
auto VerifyExplicitChartRange(
  ...
  if (const auto chart_explicit_range_result = RunDataQueryOrRecordFailure(
          ...
          "RunDataQuery(insights-chart, explicit range)", failures);
      chart_explicit_range_result.has_value()) {
    if (const auto payload =
            ParseJsonOrRecordFailure(chart_explicit_range_result->content,
                                     "insights-chart explicit range", failures);
        payload.has_value()) {
      ValidateChartSeriesPayload(*payload, "insights-chart explicit range",
                                 failures);
      const auto series_it = payload->find("series");
      if (series_it == payload->end() || !series_it->is_array() ||
          series_it->size() != 7U) {
        ++failures;
        std::cerr << "[FAIL] insights-chart explicit range should return 7 "
                     "points.\n";
      }
      if (payload->value("from_date", std::string()) !=
        ...
        ++failures;
        std::cerr << "[FAIL] insights-chart explicit range should echo "
...
auto VerifyChartForRootScenarios(
  ...
  if (const auto chart_root_result = RunDataQueryOrRecordFailure(
    ...
    if (const auto payload = ParseJsonOrRecordFailure(
      ...
      if (series_it == payload->end() || !series_it->is_array() ||
          series_it->size() != 7U) {
        ++failures;
        std::cerr << "[FAIL] insights-chart selected root should return 7 "
                     "points.\n";
      }
      if (payload->value("selected_root", std::string()) !=
          *chart_probe.selected_root) {
        ++failures;
        std::cerr << "[FAIL] insights-chart selected_root should match "
                     "request --root value.\n";
      }
    }
  }
  ...
  if (const auto result = RunDataQueryOrRecordFailure(
          ...
          "RunDataQuery(insights-chart, root overrides project)", failures);
      result.has_value()) {
    if (const auto payload = ParseJsonOrRecordFailure(
            result->content, "insights-chart root overrides project", failures);
        payload.has_value()) {
      ValidateChartSeriesPayload(
          *payload, "insights-chart root overrides project", failures);
      if (payload->value("selected_root", std::string()) !=
          *chart_probe.selected_root) {
        ++failures;
        std::cerr << "[FAIL] insights-chart should prefer request --root over "
  ...
  if (const auto result = RunDataQueryOrRecordFailure(
          ...
          "RunDataQuery(insights-chart, missing root)", failures);
      result.has_value()) {
    if (const auto payload = ParseJsonOrRecordFailure(
            result->content, "insights-chart missing root", failures);
        payload.has_value()) {
      ValidateChartSeriesPayload(*payload, "insights-chart missing root",
                                 failures);
      if (payload->value("selected_root", std::string()) != missing_root) {
        ++failures;
        std::cerr << "[FAIL] insights-chart missing root should echo "
                     "selected_root from request.\n";
      }
      if (payload->value("total_duration_seconds", -1LL) != 0LL) {
        ++failures;
        std::cerr << "[FAIL] insights-chart missing root should produce "
  ...
  if (const auto result = RunDataQueryOrRecordFailure(
          ...
          "RunDataQuery(insights-chart, empty root)", failures);
      result.has_value()) {
    if (const auto payload = ParseJsonOrRecordFailure(
  ...
  if (const auto result = RunDataQueryOrRecordFailure(
          ...
          "RunDataQuery(insights-chart, boundary)", failures);
      result.has_value()) {
    if (const auto payload = ParseJsonOrRecordFailure(
            result->content, "insights-chart boundary", failures);
        payload.has_value()) {
      ValidateChartSeriesPayload(*payload, "insights-chart boundary", failures);
      const auto series_it = payload->find("series");
      if (series_it == payload->end() || !series_it->is_array() ||
          series_it->size() != 1U) {
        ++failures;
        std::cerr << "[FAIL] insights-chart boundary should return exactly 1 "
      ...
      if (chart_probe.latest_date.has_value() && series_it != payload->end() &&
        ...
        if (first.is_object() && first.contains("date") &&
          ...
          ++failures;
          std::cerr << "[FAIL] insights-chart boundary point date should be "
                       "the latest date.\n";
        }
      }
    }
