...
auto TestValidateLogicReusesStructureReporter(int& failures) -> void {
  const RuntimeTestPaths kPaths = BuildTempTestPaths(
      "time_tracer_android_runtime_validate_logic_structure_insights_test");
  ...
  if (!WriteFileWithParents(kSourceFile, "y2026\nm03\nd0301\nr\n")) {
    ++failures;
    std::cerr << "[FAIL] Validate-logic structure insights test should write "
                 "input file.\n";
    cleanup();
    return;
  }
  ...
  if (!Contains(kLogs, "[Pipeline] 转换前结构预检失败，已跳过转换阶段。")) {
    ++failures;
    std::cerr << "[FAIL] RunValidateLogic should insights precheck failure as "
