...
void PrintFullUsage(
  ...
  for (const auto &[name, cmd_ptr] : commands) {
    ...
    const std::string category =
        time_tracer::cli::framework::core::ResolveCommandCategory(
            name, cmd_ptr->GetCategory());
    usage_name_width =
        std::max(usage_name_width, static_cast<int>(name.size()) + 1);
    grouped_commands[category].emplace_back(name, cmd_ptr.get());
  }
    ...
    if (grouped_commands.contains(category)) {
      ...
      for (const auto &[name, cmd] : grouped_commands[category]) {
        std::string description = ToSummaryDescription(
            time_tracer::cli::framework::core::ResolveCommandSummary(
                name, cmd->GetHelp()));
        std::string usage_prefix = BuildUsagePrefix(name, usage_name_width);
...
void PrintCommandUsage(const std::string &command_name,
  ...
  std::string description = NormalizeDescription(
      time_tracer::cli::framework::core::ResolveCommandSummary(
          command_name, command.GetHelp()));
