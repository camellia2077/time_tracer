...
public:
  explicit ImportCommand(ITimeTracerCoreApi &core_api);
...
ImportCommand::ImportCommand(ITimeTracerCoreApi &core_api)
    : core_api_(core_api) {}
...
auto ImportCommand::GetHelp() const -> std::string {
  return time_tracer::cli::framework::core::ResolveCommandSummary(
      "import", "Import processed JSON data into the database.");
}
