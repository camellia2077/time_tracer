...
public:
  ConvertCommand(ITimeTracerCoreApi &core_api,
...
ConvertCommand::ConvertCommand(ITimeTracerCoreApi &core_api,
                               DateCheckMode default_date_check_mode,
                               bool default_save_processed_output,
                               bool default_validate_logic,
                               bool default_validate_structure)
...
auto ConvertCommand::GetHelp() const -> std::string {
  return time_tracer::cli::framework::core::ResolveCommandSummary(
      "convert", "Convert source files (e.g., .txt) to processed JSON format.");
}
...
void ConvertCommand::Execute(const CommandParser &parser) {
  ...
  time_tracer::core::dto::ConvertRequest request;
