...
public:
  explicit ValidateStructureCommand(ITimeTracerCoreApi &core_api);
...
ValidateStructureCommand::ValidateStructureCommand(ITimeTracerCoreApi &core_api)
    : core_api_(core_api) {}
...
auto ValidateStructureCommand::GetHelp() const -> std::string {
  return time_tracer::cli::framework::core::ResolveCommandSummary(
      "validate-structure",
      "Validate the syntax and structure of source TXT files (Read-only).");
}
