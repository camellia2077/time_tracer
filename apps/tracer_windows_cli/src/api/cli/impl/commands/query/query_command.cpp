...
public:
  ...
  [[nodiscard]] auto GetCategory() const -> std::string override {
    return time_tracer::cli::framework::core::ResolveCommandCategory("query",
                                                                     "Query");
  }
...
auto QueryCommand::GetDefinitions() const -> std::vector<ArgDef> {
  return {
      ...
      {"project",
       ArgType::kOption,
       {"--project"},
       time_tracer::cli::framework::core::ResolveDeprecatedOptionHelp(
           "query", "--project",
...
auto QueryCommand::GetHelp() const -> std::string {
  return time_tracer::cli::framework::core::ResolveCommandSummary(
      "query",
      "Query statistics (day, month, week, year, recent, range) from the "
      "database. For query data tree/days-stats/report-chart, prefer --root "
      "over legacy --project.");
}
