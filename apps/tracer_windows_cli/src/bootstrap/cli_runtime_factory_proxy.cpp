// tracer_windows/src/bootstrap/cli_runtime_factory_proxy.cpp
...
using time_tracer::core::dto::OperationAck;
...
using time_tracer::core::dto::TextOutput;
using time_tracer::core::dto::TreeQueryRequest;
using time_tracer::core::dto::TreeQueryResponse;
...
[[nodiscard]] auto
ToDataOutputModeToken(time_tracer::core::dto::DataQueryOutputMode mode)
    -> std::string_view {
...
class CoreApiProxy final : public ITimeTracerCoreApi {
public:
  ...
  auto
  RunValidateLogic(const time_tracer::core::dto::ValidateLogicRequest &request)
      -> OperationAck override {
  ...
  auto RunStructuredPeriodBatchQuery(
      const time_tracer::core::dto::StructuredPeriodBatchQueryRequest
          & /*request*/)
...
private:
  ...
  auto CallAck(Fn function, std::string_view request_json,
    ...
    if (parsed.HasError()) {
      throw LogicError(parsed.error.message);
    }
    OperationAck out{};
  ...
  auto CallTree(Fn function, std::string_view request_json,
    ...
    const auto parsed =
        tt_transport::ParseResponseEnvelope(response_raw, context);
    if (parsed.HasError()) {
      throw LogicError(parsed.error.message);
    }
