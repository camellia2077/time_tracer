// infrastructure/query/data/data_query_repository_internal.hpp
#pragma once
...
[[nodiscard]] auto NeedRecordsJoinForFilters(const QueryFilters& filters)
    -> bool;

[[nodiscard]] auto BuildDayFilterClauses(
    const std::optional<int>& year, const std::optional<int>& month,
    const std::optional<std::string>& from_date,
    const std::optional<std::string>& to_date,
    std::vector<detail::SqlParam>& params) -> std::vector<std::string>;
