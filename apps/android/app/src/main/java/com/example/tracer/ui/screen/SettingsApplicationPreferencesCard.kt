...
@Composable
internal fun SettingsApplicationPreferencesCard(
    appLanguage: AppLanguage,
    onSetAppLanguage: (AppLanguage) -> Unit,
    timeDisplayMode: TimeDisplayMode,
    onTimeDisplayModeChange: (TimeDisplayMode) -> Unit,
    ...
    var timeDisplayExpanded by rememberSaveable { mutableStateOf(false) }
        ...
        Column(
            ...
            if (expanded) {
                ...
                ExpandableSettingsButton(
                    ...
                    expanded = pageTransitionOptionsExpanded,
                    onClick = {
                        onPageTransitionOptionsExpandedChange(!pageTransitionOptionsExpanded)
                    },
                    previewContent = {
                        Text(
                            text = pageTransitionStyleLabel(pageTransitionStyle),
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant
                        )
                    }
                )
                if (pageTransitionOptionsExpanded) {
                    Column(
                        modifier = Modifier
                            .fillMaxWidth()
                            .padding(top = 8.dp)
                            .testTag("settings_page_transition_options")
                    ) {
                        PageTransitionStyle.entries.forEach { style ->
                            val selected = pageTransitionStyle == style
                            Row(
                                modifier = Modifier
                                    .fillMaxWidth()
                                    .testTag("settings_page_transition_option_${style.name.lowercase()}")
                                    .selectable(
                                        selected = selected,
                                        onClick = {
                                            onPageTransitionStyleChange(style)
                                            onPageTransitionOptionsExpandedChange(true)
                                        },
                ...
                HorizontalDivider(modifier = Modifier.padding(vertical = 12.dp))
                ExpandableSettingsButton(
                    text = stringResource(R.string.config_label_time_display),
                    expanded = timeDisplayExpanded,
                    onClick = { timeDisplayExpanded = !timeDisplayExpanded },
                    previewContent = {
                        Text(
                            text = timeDisplayModeLabel(timeDisplayMode),
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant
                        )
                    }
                )
                if (timeDisplayExpanded) {
                    Column(
                        modifier = Modifier
                            .fillMaxWidth()
                            .padding(top = 8.dp)
                            .testTag("config_time_display_options")
                    ) {
                            ...
                            Row(
                                modifier = Modifier
                                    ...
                                    .selectable(
                                        ...
                                        onClick = { onTimeDisplayModeChange(mode) },
                                        role = Role.RadioButton
                                    )
                                    .padding(vertical = 6.dp),
                                verticalAlignment = androidx.compose.ui.Alignment.CenterVertically
                                ...
                                Text(
                                    text = timeDisplayModeLabel(mode),
                                    style = MaterialTheme.typography.bodyMedium,
                                    modifier = Modifier.padding(start = 8.dp)
                                )
                            }
...
private fun pageTransitionStyleLabel(style: PageTransitionStyle): String = when (style) {
    PageTransitionStyle.NONE -> stringResource(R.string.config_page_transition_none)
...
@Composable
private fun timeDisplayModeLabel(mode: TimeDisplayMode): String = when (mode) {
    TimeDisplayMode.TWENTY_FOUR_HOUR -> stringResource(R.string.config_time_display_24_hour)
    TimeDisplayMode.TWELVE_HOUR -> stringResource(R.string.config_time_display_12_hour)
}
