...
internal fun parseHeatmapPoints(points: List<InsightsChartPoint>): List<ParsedHeatmapPoint> =
...
internal fun buildHeatmapPlot(
    ...
    return when (mode) {
        InsightsHeatmapMode.MONTH -> HeatmapPlot(
...
internal fun normalizeThresholds(rawThresholds: List<Double>): List<Double> {
    if (rawThresholds.isEmpty()) {
        return defaultInsightsHeatmapTomlConfig().thresholdsHours
    }
...
internal fun resolvePaletteName(
    ...
    if (availableNames.isEmpty()) {
        val fallbackConfig = defaultInsightsHeatmapTomlConfig()
        return fallbackConfig.defaultLightPalette
    }
    if (stylePreference.themePolicy == InsightsHeatmapThemePolicy.PALETTE &&
    ...
    val autoPaletteName = when (stylePreference.themePolicy) {
        ...
        InsightsHeatmapThemePolicy.PALETTE -> config.defaultLightPalette
    }
