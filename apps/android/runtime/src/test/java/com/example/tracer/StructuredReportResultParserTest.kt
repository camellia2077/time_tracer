...
class StructuredReportResultParserTest {
    private val parser = StructuredReportResultParser()
    ...
    fun parse_mapsCoreRecordKindToTimelineModel() {
        ...
        assertEquals(
            ...
            result.report?.activities?.single()?.kind
        )
        assertEquals(0L, result.report?.activities?.single()?.durationSeconds)
    }
    ...
    private fun structuredResponse(recordKind: String?): String {
        ...
        return """
            {"ok":true,"report":{"date":"2026-03-20","total_duration":0,
