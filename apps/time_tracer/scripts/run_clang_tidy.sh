...
"${PYTHON_BIN[@]}" "${REPO_ROOT}/scripts/run.py" configure --app time_tracer --build-dir "${BUILD_DIR}" --tidy

if [[ "${MODE}" == "check" ]]; then
  # Use `run.py tidy` to preserve build.log + tasks/batch_* splitting workflow.
  "${PYTHON_BIN[@]}" "${REPO_ROOT}/scripts/run.py" tidy --app time_tracer --build-dir "${BUILD_DIR}" "${KEEP_GOING_ARGS[@]}" -- --target tidy
else
  BUILD_EXTRA_ARGS=(--target tidy-fix)
  if [[ "${KEEP_GOING}" == "true" ]]; then
    BUILD_EXTRA_ARGS+=(-- -k 0)
  fi
  "${PYTHON_BIN[@]}" "${REPO_ROOT}/scripts/run.py" build --app time_tracer --build-dir "${BUILD_DIR}" --tidy -- "${BUILD_EXTRA_ARGS[@]}"
fi
