...
$workflowDefault = Join-Path $repoRoot ".agent\workflows\tt-tidy-all.md"
...
Hard constraints:
1) Only finish when no `task_*.log` exists under `apps/time_tracer/build_tidy/tasks/`.
2) Keep `test/output/time_tracer/result.json` with `success: true`.
