...
#ifndef CLI_FRAMEWORK_CORE_ARG_DEFINITIONS_HPP_
#define CLI_FRAMEWORK_CORE_ARG_DEFINITIONS_HPP_
...
struct ArgDef {
    ...
    ArgType type;
    std::vector<std::string> keys; // 触发键，如 {"-f", "--format"} (仅 Option/Flag 有效)
    std::string help;           // 帮助文本
    bool required = false;
    std::string default_value = "";
    
    // 用于位置参数：指定它在 filtered_args 中的索引（偏移量，跳过程序名和命令名后）
    // 例如：convert <path> -> index 0
