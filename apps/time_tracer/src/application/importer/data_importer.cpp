...
#include "application/importer/data_importer.hpp"
...
void PrintReport(const ImportStats& stats, const std::string& title) {
  ...
  if (stats.failed_files.empty()) {
    std::cout << colors::kGreen << "[Success] Processed "
              << stats.successful_files << " items." << colors::kReset
              << std::endl;
  } else {
    std::cout << colors::kYellow
              << "[Partial] Success: " << stats.successful_files
