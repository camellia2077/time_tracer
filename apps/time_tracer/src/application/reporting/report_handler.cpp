...
auto ReportHandler::run_daily_query(const std::string& date,
                                    ReportFormat format) -> std::string {
  return generator_->generate_daily_report(date, format);
}

auto ReportHandler::run_monthly_query(const std::string& month,
                                      ReportFormat format) -> std::string {
...
// [新增] 批量查询逻辑下沉至 Core
auto ReportHandler::run_period_queries(const std::vector<int>& days_list,
                                       ReportFormat format) -> std::string {
...
void ReportHandler::run_export_single_day_report(const std::string& date,
                                                 ReportFormat format) {
  auto content = generator_->generate_daily_report(date, format);
  // [修复] 调用 export_single_day_report (之前错误调用了
  // export_all_daily_reports)
  exporter_->export_single_day_report(date, content, format);
}

void ReportHandler::run_export_single_month_report(const std::string& month,
                                                   ReportFormat format) {
...
void ReportHandler::run_export_single_period_report(int days,
                                                    ReportFormat format) {
  auto content = generator_->generate_period_report(days, format);
  // [修复] 调用 export_single_period_report (之前拼写错误或不存在
...
void ReportHandler::run_export_all_period_reports_query(
