...
void Writer::insert_days(const std::vector<DayData>& days) {
  ...
  for (const auto& day_data : days) {
    sqlite3_bind_text(stmt_insert_day_, 1, day_data.date.c_str(), -1,
                      SQLITE_TRANSIENT);
    ...
    sqlite3_bind_text(stmt_insert_day_, 6, day_data.remark.c_str(), -1,
                      SQLITE_TRANSIENT);

    if (day_data.getup_time == "Null" || day_data.getup_time.empty()) {
      sqlite3_bind_null(stmt_insert_day_, 7);
    } else {
      sqlite3_bind_text(stmt_insert_day_, 7, day_data.getup_time.c_str(), -1,
                        SQLITE_TRANSIENT);
    }
...
void Writer::insert_records(const std::vector<TimeRecordInternal>& records) {
  ...
  for (const auto& record_data : records) {
    ...
    sqlite3_bind_text(stmt_insert_record_, 4, record_data.date.c_str(), -1,
                      SQLITE_TRANSIENT);

    sqlite3_bind_text(stmt_insert_record_, 5,
                      record_data.start_time_str.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt_insert_record_, 6, record_data.end_time_str.c_str(),
                      -1, SQLITE_TRANSIENT);
    ...
    if (record_data.remark.has_value()) {
      sqlite3_bind_text(stmt_insert_record_, 9, record_data.remark->c_str(), -1,
                        SQLITE_TRANSIENT);
    } else {
