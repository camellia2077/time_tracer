...
void DisplayHeader(std::stringstream& report_stream,
    ...
    // 1. 渲染标题
    std::string title_content = config->GetReportTitle() + " " + TexUtils::EscapeLatex(data.date);
    ...
    std::vector<TexCommonUtils::SummaryItem> items = {
        ...
        {config->GetTotalTimeLabel(), TexUtils::EscapeLatex(TimeFormatDuration(data.total_duration))},
        {config->GetStatusLabel(),     TexUtils::EscapeLatex(BoolToString(data.metadata.status))},
        {config->GetSleepLabel(),      TexUtils::EscapeLatex(BoolToString(data.metadata.sleep))},
        {config->GetExerciseLabel(),   TexUtils::EscapeLatex(BoolToString(data.metadata.exercise))},
        {config->GetGetupTimeLabel(), TexUtils::EscapeLatex(data.metadata.getup_time)},
...
void DisplayDetailedActivities(std::stringstream& report_stream,
    ...
    for (const auto& record : data.detailed_records) {
        std::string project_path = ReplaceAll(record.project_path, "_", config->GetActivityConnector());
        std::string base_string = TexUtils::EscapeLatex(record.start_time) + " - " +
        ...
        if (record.activityRemark.has_value()) {
            ...
            const std::string& activity_remark = *record.activityRemark;

            std::string safe_activity_remark =
                TexUtils::EscapeLatex(activity_remark);
