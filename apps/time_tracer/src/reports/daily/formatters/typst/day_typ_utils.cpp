        ...
        for (const auto& pair : config->GetKeywordColors()) {
            if (record.project_path.find(pair.first) != std::string::npos) {
                ...
                if (record.activityRemark.has_value()) {
                    ...
                    std::string formatted_activity_remark = FormatMultilineForList(
                        record.activityRemark.value(), 
                        4,     // 缩进 4 个空格
                        " \\"  // 行尾追加 " \"
                    );
                    final_output += std::format("\n  + *{}:* {}", config->GetActivityRemarkLabel(), formatted_activity_remark);
                }
                return final_output;
            }
        }
...
namespace DayTypUtils {
    ...
    void DisplayHeader(std::stringstream& report_stream, const DailyReportData& data, const std::shared_ptr<DayTypConfig>& config) {
        std::string title = std::format(
            ...
            config->GetTitlePrefix(),
            data.date
        );
        report_stream << title << "\n\n";
        report_stream << std::format("+ *{}:* {}\n", config->GetDateLabel(), data.date);
        report_stream << std::format("+ *{}:* {}\n", config->GetTotalTimeLabel(), TimeFormatDuration(data.total_duration));
        report_stream << std::format("+ *{}:* {}\n", config->GetStatusLabel(), BoolToString(data.metadata.status));
        report_stream << std::format("+ *{}:* {}\n", config->GetSleepLabel(), BoolToString(data.metadata.sleep));
        report_stream << std::format("+ *{}:* {}\n", config->GetExerciseLabel(), BoolToString(data.metadata.exercise));
        report_stream << std::format("+ *{}:* {}\n", config->GetGetupTimeLabel(), data.metadata.getup_time);
