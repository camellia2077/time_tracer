...
void AppendTreeChildren(std::ostringstream& output_stream,
  ...
  const auto children = CollectSortedChildren(node);
  for (size_t index = 0; index < children.size(); ++index) {
    const auto& child = children[index];
    const bool is_last = (index + 1 == children.size());
    output_stream << prefix << (is_last ? "└── " : "├── ") << child.name
                  << "\n";
    const std::string next_prefix = prefix + (is_last ? "    " : "│   ");
    AppendTreeChildren(output_stream, *child.node, next_prefix,
...
auto RenderActivitySuggestions(const std::vector<ActivitySuggestionRow>& rows)
    -> std::string {
  std::ostringstream output_stream;
  output_stream << std::fixed << std::setprecision(2);
  for (const auto& row : rows) {
...
auto PrintActivitySuggestions(const std::vector<ActivitySuggestionRow>& rows)
    -> void {
  time_tracer::domain::ports::EmitInfo(RenderActivitySuggestions(rows));
}

auto RenderProjectTree(const reporting::ProjectTree& tree, int max_depth)
    -> std::string {
  std::ostringstream output_stream;
  if (tree.empty()) {
...
auto PrintProjectTree(const reporting::ProjectTree& tree, int max_depth)
    -> void {
  time_tracer::domain::ports::EmitInfo(RenderProjectTree(tree, max_depth));
}
