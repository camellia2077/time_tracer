...
void RangeMdFormatter::FormatHeaderContent(std::stringstream& report_stream,
...
__declspec(dllexport) void DestroyFormatter(FormatterHandle handle) {  // NOLINT
  if (handle != nullptr) {
    std::unique_ptr<RangeMdFormatter>{static_cast<RangeMdFormatter*>(handle)};
  }
}
...
formatReport_Legacy(  // NOLINT(readability-identifier-naming)
    FormatterHandle handle, const RangeReportData& data) -> const
    char* {  // NOLINT
