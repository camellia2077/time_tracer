...
auto CreateFormatterImpl(const TtFormatterConfig* formatter_config,
                         RangeMdFormatter** formatter_out)
  ...
  try {
    const TtRangeMdConfigV1* config_view = nullptr;
    std::string config_error;
    if (!formatter_c_config_bridge_v1::GetRangeMdConfigView(
    ...
    auto md_config = std::make_shared<RangeMdConfig>(*config_view);
...
auto FormatReportImpl(RangeMdFormatter* formatter,
...
__declspec(dllexport) auto tt_createFormatter(const TtFormatterConfig* config,
  ...
  RangeMdFormatter* formatter = nullptr;
...
__declspec(dllexport) auto tt_formatReport(TtFormatterHandle handle,
  ...
  *out_report_size = 0;
  ...
  if (!formatter_c_config_bridge_v1::ValidateReportDataView(
          ...
          kSupportedReportDataKinds.size(), "range markdown",
