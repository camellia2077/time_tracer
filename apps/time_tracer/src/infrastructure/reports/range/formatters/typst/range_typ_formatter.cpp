...
auto CreateFormatterImpl(const TtFormatterConfig* formatter_config,
                         RangeTypFormatter** formatter_out)
  ...
  try {
    const TtRangeTypConfigV1* config_view = nullptr;
    std::string config_error;
    if (!formatter_c_config_bridge_v1::GetRangeTypConfigView(
    ...
    auto typ_config = std::make_shared<RangeTypConfig>(*config_view);
...
auto FormatReportImpl(RangeTypFormatter* formatter,
...
__declspec(dllexport) auto tt_createFormatter(const TtFormatterConfig* config,
  ...
  RangeTypFormatter* formatter = nullptr;
...
__declspec(dllexport) auto tt_formatReport(TtFormatterHandle handle,
  ...
  *out_report_size = 0;
  ...
  if (!formatter_c_config_bridge_v1::ValidateReportDataView(
          ...
          kSupportedReportDataKinds.size(), "range typst",
