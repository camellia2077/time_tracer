...
auto LogSerializer::serialize(const DailyLog& day) -> nlohmann::json {
  ...
  headers_obj[json_keys::kExercise] =
      static_cast<int>(day.hasExerciseActivity);
  ...
  headers_obj[json_keys::kAnaerobic] =
      (day.stats.anaerobic_time > 0) ? 1 : 0;
  ...
  for (const auto& activity_data : day.processedActivities) {
    ...
    activity_obj[json_keys::kDurationSeconds] =
        activity_data.duration_seconds;

    if (activity_data.remark.has_value()) {
      activity_obj[json_keys::kActivityRemark] =
          activity_data.remark.value();
    } else {
  ...
  generated_stats_obj[json_keys::kRecreationTime] =
