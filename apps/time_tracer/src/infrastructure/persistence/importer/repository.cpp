...
auto Repository::ReplaceMonthData(
    const int year, const int month, const std::vector<DayData>& days,
  ...
  const auto boundary = BuildMonthBoundary(year, month);
  ...
  try {
    const std::string delete_records_sql = std::format(
        ...
        boundary->start_date, boundary->next_month_start_date);
...
auto Repository::TryGetLatestActivityTailBeforeDate(std::string_view date) const
  ...
  if (sqlite3_prepare_v2(connection_manager_->GetDb(), sql.c_str(), -1, &stmt,
                         nullptr) != SQLITE_OK) {
    return std::nullopt;
  }
