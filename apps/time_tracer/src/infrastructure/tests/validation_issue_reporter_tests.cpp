...
class CapturingDiagnosticsSink final
 ...
 public:
  auto Emit(time_tracer::domain::ports::DiagnosticSeverity severity,
    ...
    if (severity == time_tracer::domain::ports::DiagnosticSeverity::kError) {
...
class DiagnosticsStateGuard {
 public:
  DiagnosticsStateGuard()
      : previous_sink_(time_tracer::domain::ports::GetDiagnosticsSink()),
        previous_writer_(time_tracer::domain::ports::GetErrorReportWriter()) {}

  ~DiagnosticsStateGuard() {
    time_tracer::domain::ports::SetDiagnosticsSink(previous_sink_);
    time_tracer::domain::ports::SetErrorReportWriter(previous_writer_);
  }

 private:
  std::shared_ptr<time_tracer::domain::ports::IDiagnosticsSink> previous_sink_;
...
auto InstallCapturingPorts()
  ...
  time_tracer::domain::ports::SetDiagnosticsSink(sink);
  time_tracer::domain::ports::SetErrorReportWriter(writer);
  return {sink, writer};
}
