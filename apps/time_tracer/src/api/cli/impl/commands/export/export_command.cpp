...
void RunExportWithArgument(IReportHandler& report_handler,
  ...
  if (args.sub_command == "day") {
    report_handler.RunExportSingleDayReport(std::string(args.export_arg),
                                            format);
  } else if (args.sub_command == "month") {
    report_handler.RunExportSingleMonthReport(std::string(args.export_arg),
                                              format);
  } else if (args.sub_command == "week") {
    report_handler.RunExportSingleWeekReport(std::string(args.export_arg),
                                             format);
  } else if (args.sub_command == "year") {
    report_handler.RunExportSingleYearReport(std::string(args.export_arg),
                                             format);
    ...
    std::vector<int> days_list =
        ArgUtils::ParseNumberList(std::string(args.export_arg));
  ...
  for (const auto& format : kFormats) {
    if (RequiresExportArgument(kSubCommand)) {
      ...
      RunExportWithArgument(
          report_handler_,
          {.sub_command = kSubCommand, .export_arg = kNormalizedArg}, format);
    } else {
