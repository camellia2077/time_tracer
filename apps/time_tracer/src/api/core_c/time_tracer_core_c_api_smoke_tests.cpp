...
auto FindRepoRoot() -> std::filesystem::path {
  ...
  while (!current.empty()) {
    const fs::path kConfigProbe = current / "apps" / "time_tracer" / "config" /
                                  "converter" /
...
auto main() -> int {
  ...
  try {
    ...
    const std::filesystem::path kTempRoot =
        kRepoRoot / "test" / "output" / "time_tracer_core_c_api_smoke";
    ...
    std::cout << "[PASS] time_tracer_core_c_api_smoke_tests version=" << version
              << '\n';
    CloseLibrary(library);
    return 0;
  } catch (const std::exception& error) {
    ...
    std::cerr << "[FAIL] time_tracer_core_c_api_smoke_tests: " << error.what()
              << '\n';
    return 1;
  } catch (...) {
    if (library != nullptr) {
    ...
    std::cerr << "[FAIL] time_tracer_core_c_api_smoke_tests: unknown error\n";
    return 1;
  }
}
