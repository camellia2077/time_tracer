...
const std::array<JNINativeMethod, 7> kNativeMethods = {
    JNINativeMethod{
...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* /*reserved*/) {
  ...
  for (const char* class_name : kBridgeClassCandidates) {
    if (time_tracer::api::android::bridge_internal::TryRegisterNativeMethods(
            env, class_name)) {
      registered = true;
      break;
    }
...
extern "C" JNIEXPORT void JNICALL JNI_OnUnload(JavaVM* /*vm*/,
  ...
  std::scoped_lock lock(
      time_tracer::api::android::bridge_internal::g_runtime_mutex);
  time_tracer::api::android::bridge_internal::DestroyRuntimeLocked();
}
