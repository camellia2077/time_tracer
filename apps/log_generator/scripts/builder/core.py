...
from .ui.console import print_error, print_header, print_success
from .ui.colors import AnsiColors
from .utils.logger import BuildLogger


class BuildPipeline:
    """Core controller for the build process."""
    ...
    def run(self, argv: list):
        ...
        try:
            # 1. Load config
            config_path = self.script_dir / "build.toml"
    ...
    def _execute_steps(self, cmake_args):
        ...
        cmake_invoker.run_cmake(
            ...
            self.config,
            self.logger,
            no_opt=self.options.get("no_opt"),
            no_tidy=self.options.get("no_tidy"),
            no_lto=self.options.get("no_lto"),
            no_warn=self.options.get("no_warn"),
            no_pch=self.options.get("no_pch"),
            fail_fast=self.options.get("fail_fast"),
            quiet=self.options.get("quiet"),
        )

        target = ("tidy-fix" if self.options.get("fix") else "tidy") if self.options.get("tidy") else None
        build_log, build_success = cmake_invoker.run_build(
            logger=self.logger,
            target=target,
            quiet=self.options.get("quiet"),
        )
