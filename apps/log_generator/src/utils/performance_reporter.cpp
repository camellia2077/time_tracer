...
void PerformanceReporter::add_generation_time(
    const std::chrono::nanoseconds& duration) {
  std::scoped_lock lock(mutex_);
  total_generation_duration_ += duration;
}

void PerformanceReporter::add_io_time(
    const std::chrono::nanoseconds& duration) {
  std::scoped_lock lock(mutex_);
  total_io_duration_ += duration;
}

void PerformanceReporter::report(
    const Config& config, int files_generated,
  ...
  auto gen_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
      total_generation_duration_);

  auto io_s = std::chrono::duration<double>(total_io_duration_);
  auto io_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(total_io_duration_);
  ...
  std::cout << std::format(
      "Cumulative Gen Time:  {:.2f} s ({}ms) [Sum of all threads]\n",
      gen_s.count(), gen_ms.count());
  std::cout << std::format(
      "Cumulative IO Time:   {:.2f} s ({}ms) [Sum of all threads]\n",
