...
`ui/branding` 用于存放 Time Tracer 的长期品牌设计参考资源。
...
`ui/branding` 现在明确分为三层：

1. `ui/branding/master/`
   - 品牌母版层
   - 保存跨平台共享的核心几何、比例、留白和识别语义
2. `ui/branding/platform/`
   - 平台适配层
   - 说明各平台如何从品牌母版落地为最终资源
3. `ui/branding/explorations/`
   - 设计探索层
   - 保存历史探索稿、实验稿、候选稿
...
- Android
  - 参考无背景品牌母版几何
  - 默认平台参考：`ui/branding/platform/android/time_tracer_android_launcher_reference.svg`
  - 最终落地为 Android adaptive icon 的 `background / foreground / monochrome` 三个资源
- Windows CLI
  - 参考无背景品牌母版几何
  - 默认构建源：`ui/branding/exports/bg_white_vertical_padding_rounded.svg`
  - 最终在构建期转为 `.ico`

平台映射文档：

- `ui/branding/platform/android/README.md`
- `ui/branding/platform/windows_cli/README.md`
...
6. 新的长期品牌资源，统一收口到 `ui/branding/**`
7. 不再在仓库其他位置新增长期共享设计资源目录
8. 交互式设计演示文件位于：
   - `ui/branding/explorations/demo/demo.html`

## 平台落地文档

- Android 图标 SVG 应用说明：
  - `ui/branding/android-icon-svg-application.md`
- Windows CLI 图标 SVG 应用说明：
  - `ui/branding/windows-cli-icon-svg-application.md`
