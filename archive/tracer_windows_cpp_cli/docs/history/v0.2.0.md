...
### Windows CLI 图标接入（Release）
1. Windows CLI 新增箭头品牌图标资源：
   - `apps/shared/branding/arrow.svg`（共享源）
   - `archive/tracer_windows_cpp_cli/src/resources/app_icon.rc`
   - `archive/tracer_windows_cpp_cli/src/resources/output_icon.ico`
2. `tracer_windows_cli` 的 release 构建自动开启 icon 注入（`ENABLE_APP_ICON=ON`）；非 release 配置保持不注入。
3. 旧的 `apps/tracer_core/src/resources` 图标资源已移除，避免 core 层继续承载 CLI 品牌资源。
...
1. CMake 已接入新模块：
   - `apps/tracer_cli/windows/src/api/cli/CMakeLists.txt`
2. 回归验证：
   - `python tools/run.py post-change --app tracer_windows_cli`
