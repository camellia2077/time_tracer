...
public:
  ValidateLogicCommand(ITracerCoreApi &core_api,
  ...
  [[nodiscard]] auto GetCategory() const -> std::string override {
    return tracer_core::cli::framework::core::ResolveCommandCategory(
        "validate-logic", ICommand::GetCategory());
  }
...
ValidateLogicCommand::ValidateLogicCommand(
    ITracerCoreApi &core_api, DateCheckMode default_date_check_mode)
...
auto ValidateLogicCommand::GetHelp() const -> std::string {
  return tracer_core::cli::framework::core::ResolveCommandSummary(
      "validate-logic", ICommand::GetHelp());
}
