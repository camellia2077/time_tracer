// infrastructure/io/utils/file_utils.hpp
#ifndef INFRASTRUCTURE_IO_UTILS_FILE_UTILS_H_
#define INFRASTRUCTURE_IO_UTILS_FILE_UTILS_H_
 ...
 */
auto FindFilesByExtensionRecursively(const std::filesystem::path& root_path,
                                     const std::string& extension)
    -> std::vector<std::filesystem::path>;

/**
 * @brief [新增]
 * 解析输入路径列表，将其中的目录展开为文件，并保留直接指定的文件。
 * @details 这是一个高层辅助函数，逻辑迁移自原 db_inserter/PathScanner。
 * @param input_paths 输入的路径列表（可以是文件 or 目录）。
...
#endif  // INFRASTRUCTURE_IO_UTILS_FILE_UTILS_H_
