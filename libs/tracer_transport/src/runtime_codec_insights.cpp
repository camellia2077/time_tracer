...
auto EncodeInsightsResponse(const InsightsResponsePayload& response)
    -> std::string {
  return json{
      {"ok", response.ok},
...
auto DecodeTemporalInsightsRequest(std::string_view request_json)
  ...
  TemporalInsightsRequestPayload out{};
...
auto DecodeInsightsBatchRequest(std::string_view request_json)
  ...
  InsightsBatchRequestPayload out{};
  out.days_list = *kDaysList.value;
  out.format = kFormat.value;
  return out;
}

auto EncodeInsightsBatchRequest(const InsightsBatchRequestPayload& request)
    -> std::string {
  json payload = {
...
auto EncodeInsightsBatchResponse(const InsightsBatchResponsePayload& response)
    -> std::string {
  return json{
      {"ok", response.ok},
      {"content", response.content},
