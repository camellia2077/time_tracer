// infra/reports/shared/formatters/templates/base_md_formatter.hpp
#ifndef INFRASTRUCTURE_REPORTS_SHARED_FORMATTERS_TEMPLATES_BASE_MD_FORMATTER_H_
#define INFRASTRUCTURE_REPORTS_SHARED_FORMATTERS_TEMPLATES_BASE_MD_FORMATTER_H_

#include <memory>
#include <string>

#include "infra/reports/shared/formatters/markdown/markdown_formatter.hpp"
...
class BaseMdFormatter : public IReportFormatter<ReportDataT> {
 ...
 protected:
  ...
  virtual void FormatHeaderContent(std::string& report_stream,
                                   const ReportDataT& data) const = 0;

  // [修改] 注释掉未使用参数
  virtual void FormatExtraContent(std::string& /*report_stream*/,
                                  const ReportDataT& /*data*/) const {}

  // [修改] 改为纯虚函数，移除导致编译错误的默认实现
  [[nodiscard]] virtual auto GetNoRecordsMsg() const -> std::string = 0;

  virtual void FormatProjectTreeSection(std::string& report_stream,
                                        const ReportDataT& data) const {
    report_stream += "\n## ";
...
#endif  // INFRASTRUCTURE_REPORTS_SHARED_FORMATTERS_TEMPLATES_BASE_MD_FORMATTER_H_
