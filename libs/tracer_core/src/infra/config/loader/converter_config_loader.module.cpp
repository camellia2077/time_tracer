...
auto ConverterConfigLoader::ParseMappings(const toml::table& tbl,
  ...
  auto load_map =
    ...
    target.clear();
    for (const auto& [entry_key, entry_value_node] : *map_tbl) {
      const std::string kEntryKey = std::string(entry_key.str());
      if (kEntryKey.empty()) {
        throw std::runtime_error("Invalid converter config: '" + key +
                                 "' contains an empty key.");
      }

      const auto kEntryValue = entry_value_node.value<std::string>();
