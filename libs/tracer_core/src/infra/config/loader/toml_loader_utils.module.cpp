...
module tracer.core.infrastructure.config.loader.toml_loader_utils;
...
void FillDailyLabels(const toml::table& tbl, DailyInsightsLabels& labels) {
  ...
  if (auto value = tbl["title_prefix"].value<std::string>()) {
    labels.insights_title_prefix = *value;
  } else {
...
void FillKeywordColors(const toml::table& tbl,
  ...
  if (const toml::table* color_tbl = tbl["keyword_colors"].as_table()) {
    for (const auto& [color_key, color_value] : *color_tbl) {
      if (auto hex_color = color_value.value<std::string>()) {
