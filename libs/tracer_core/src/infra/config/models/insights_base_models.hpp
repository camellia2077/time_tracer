// infra/config/models/insights_base_models.hpp
#ifndef INFRASTRUCTURE_CONFIG_MODELS_INSIGHTS_BASE_MODELS_H_
#define INFRASTRUCTURE_CONFIG_MODELS_INSIGHTS_BASE_MODELS_H_

#include <string>
#include <vector>

// 统计项定义
struct InsightsStatisticsItem {
  std::string label;
  bool show = true;
  std::string db_column;
  std::vector<InsightsStatisticsItem> sub_items;
};
...
struct FontConfig {
  ...
  int insights_title_font_size = kDefaultTitleFontSize;
...
#endif  // INFRASTRUCTURE_CONFIG_MODELS_INSIGHTS_BASE_MODELS_H_