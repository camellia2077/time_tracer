// infrastructure/platform/android/android_platform_clock.cpp
#include "infra/platform/android/android_platform_clock.hpp"
...
auto AndroidPlatformClock::TodayLocalDateIso() const -> std::string {
  const std::time_t kNow = std::time(nullptr);
  return FormatDateTm(ToLocalTm(kNow));
}

auto AndroidPlatformClock::LocalUtcOffsetMinutes() const -> int {
  const std::time_t kNow = std::time(nullptr);
