...
auto WithReportService(
  ...
  ReportService report_service(EnsureReadableDbConnection(db_path, db_manager),
                               *report_catalog, platform_clock);
...
LazySqliteReportQueryService::LazySqliteReportQueryService(
  ...
  if (db_path_.empty()) {
    throw std::invalid_argument(
        "LazySqliteReportQueryService db_path is empty.");
  }
  if (!report_catalog_) {
    throw std::invalid_argument(
...
auto LazySqliteReportQueryService::RunDailyQuery(std::string_view date_str,
  ...
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> std::string {
        return report_service.RunDailyQuery(date_str, format);
      });
}
...
auto LazySqliteReportQueryService::RunMonthlyQuery(
  ...
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> std::string {
        return report_service.RunMonthlyQuery(year_month_str, format);
      });
}

auto LazySqliteReportQueryService::RunWeeklyQuery(std::string_view iso_week_str,
                                                  ReportFormat format) const
    -> std::string {
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> std::string {
        return report_service.RunWeeklyQuery(iso_week_str, format);
      });
}

auto LazySqliteReportQueryService::RunYearlyQuery(std::string_view year_str,
                                                  ReportFormat format) const
    -> std::string {
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> std::string {
        return report_service.RunYearlyQuery(year_str, format);
      });
}
...
auto LazySqliteReportQueryService::RunExportAllDailyReportsQuery(
  ...
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> FormattedGroupedReports {
        return report_service.RunExportAllDailyReportsQuery(format);
      });
}
...
auto LazySqliteReportQueryService::RunExportAllMonthlyReportsQuery(
  ...
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> FormattedMonthlyReports {
        return report_service.RunExportAllMonthlyReportsQuery(format);
      });
}
...
auto LazySqliteReportQueryService::RunExportAllPeriodReportsQuery(
  ...
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> FormattedPeriodReports {
        return report_service.RunExportAllPeriodReportsQuery(days_list, format);
      });
}
...
auto LazySqliteReportQueryService::RunExportAllWeeklyReportsQuery(
  ...
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> FormattedWeeklyReports {
        return report_service.RunExportAllWeeklyReportsQuery(format);
      });
}
...
auto LazySqliteReportQueryService::RunExportAllYearlyReportsQuery(
  ...
  return WithReportService(
      db_path_, report_catalog_, platform_clock_,
      [&](const ReportService& report_service) -> FormattedYearlyReports {
        return report_service.RunExportAllYearlyReportsQuery(format);
      });
}
