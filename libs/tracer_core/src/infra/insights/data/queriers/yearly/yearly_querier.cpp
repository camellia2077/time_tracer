// infra/insights/data/queriers/yearly/yearly_querier.cpp
...
void YearQuerier::HandleInvalidInput(YearlyInsightsData& data) const {
  data.is_valid = false;
}

void YearQuerier::PrepareData(YearlyInsightsData& data) const {
  int gregorian_year = 0;
...
auto BatchYearDataFetcher::FetchAllData()
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    YearlyInsightsData& data = results[year_str];
