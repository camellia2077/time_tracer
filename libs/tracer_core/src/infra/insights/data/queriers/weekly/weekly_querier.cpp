// infra/insights/data/queriers/weekly/weekly_querier.cpp
...
void WeekQuerier::HandleInvalidInput(WeeklyInsightsData& data) const {
  data.is_valid = false;
}

void WeekQuerier::PrepareData(WeeklyInsightsData& data) const {
  IsoWeek week{};
...
auto BatchWeekDataFetcher::FetchAllData()
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    WeeklyInsightsData& data = results[week_row->week_label];
