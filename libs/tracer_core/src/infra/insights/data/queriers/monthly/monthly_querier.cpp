// infra/insights/data/queriers/monthly/monthly_querier.cpp
...
void MonthQuerier::HandleInvalidInput(MonthlyInsightsData& data) const {
  data.is_valid = false;
}

void MonthQuerier::PrepareData(MonthlyInsightsData& data) const {
...
auto BatchMonthDataFetcher::FetchAllData()
  ...
  insights::data::batch::FinalizeGroupedAggregationWithDays(
...
void BatchMonthDataFetcher::FetchProjectStats(
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    MonthlyInsightsData& data = all_months_data[year_month];
