...
#include "infra/insights/export_utils.hpp"
...
auto GetInsightsFormatDetails(InsightsFormat format)
    -> std::optional<InsightsFormatDetails> {
  switch (format) {
    case InsightsFormat::kMarkdown:
      return InsightsFormatDetails{.dir_name = "markdown", .extension = ".md"};
    case InsightsFormat::kLaTeX:
      return InsightsFormatDetails{.dir_name = "latex", .extension = ".tex"};
    case InsightsFormat::kTyp:
      return InsightsFormatDetails{.dir_name = "typ", .extension = ".typ"};
  ...
  try {
    ...
    if (files_created > 0) {
      ...
      modports::EmitInfo("成功: 共创建 " + std::to_string(files_created) +
                         " 个" + insights_type_name_singular +
                         "文件，已保存至: " + final_path.string());
    } else {
      modports::EmitWarn("信息: 没有可导出的" + insights_type_name_singular +
                         "内容。");
    }
