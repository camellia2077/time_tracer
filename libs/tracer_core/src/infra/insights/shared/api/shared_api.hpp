// infra/insights/shared/api/shared_api.hpp
#ifndef INFRASTRUCTURE_INSIGHTS_SHARED_API_SHARED_API_H_
#define INFRASTRUCTURE_INSIGHTS_SHARED_API_SHARED_API_H_

// ----------------------------------------------------------------------
// 定义导入/导出宏 (适用于 Windows/MSVC 和 MinGW)
// ----------------------------------------------------------------------
#ifdef _WIN32
#ifdef INSIGHTS_SHARED_EXPORTS
// 正在构建 insights_shared.dll 本身，我们需要导出 (export)
#define INSIGHTS_SHARED_API /* NOLINT(readability-identifier-naming) */ \
  __declspec(dllexport)
#else
// 正在构建 *依赖* insights_shared.dll 的代码 (例如插件DLL)，
// 我们需要导入 (import)
#define INSIGHTS_SHARED_API /* NOLINT(readability-identifier-naming) */ \
...
#endif  // INFRASTRUCTURE_INSIGHTS_SHARED_API_SHARED_API_H_