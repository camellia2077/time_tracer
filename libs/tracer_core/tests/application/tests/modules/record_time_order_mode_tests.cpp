...
auto TestLogicalDayModeCrossMidnight(TestState& state) -> void {
  ...
  Expect(state, IsStrictlyAfter("0009", "2058", TimeOrderMode::kLogicalDay0600),
  ...
  Expect(state, IsStrictlyAfter("0200", "2207", TimeOrderMode::kLogicalDay0600),
...
auto TestNormalDaySequence(TestState& state) -> void {
  ...
  constexpr std::array<std::string_view, 5> kTimes = {"0656", "0904", "1307",
                                                      "1920", "2207"};
...
auto TestLogicalDayBoundaryAndEquality(TestState& state) -> void {
  ...
  Expect(state, IsStrictlyAfter("0559", "2359", TimeOrderMode::kLogicalDay0600),
         "logical_day_0600 should roll times before 0600 into the next logical "
         "day.");
  Expect(state,
         !IsStrictlyAfter("0600", "2359", TimeOrderMode::kLogicalDay0600),
         "logical_day_0600 should keep 0600 anchored to the same logical day "
         "boundary.");
  Expect(state,
  ...
  Expect(
      state, !IsStrictlyAfter("0904", "0904", TimeOrderMode::kStrictCalendar),
  ...
  Expect(state,
         ...
         "logical_day_0600 should reject equal timestamps as strictly "
         "increasing.");
